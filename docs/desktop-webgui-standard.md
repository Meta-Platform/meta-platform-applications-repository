...
3. `@/i-components.icomponents` fornece tokens, temas, CSS, primitivas e estado
   comuns.
4. Uma biblioteca de módulo, como
   `@/instance-manager.icomponents`, contém apenas componentes exclusivos da
   área.
...
│   └── src/
└── feature.icomponents/       # somente quando há UI exclusiva reutilizável
...
`i-components.icomponents`; componentes usados por vários aplicativos do mesmo
módulo ficam na `.icomponents` desse módulo.

## Critério para o catálogo

Cada `.icomponents` publica histórias executáveis. Cada WebGui possui uma
