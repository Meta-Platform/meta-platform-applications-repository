...
const MutationResult = (data: any, view: any, entity: any) => {
...
const ListEnvelope = ({ rows, total, limit, offset, fields, defaultFields }: any) => {
    const projection = Array.isArray(fields) && fields.length ? fields : defaultFields
    const items = projection ? rows.map((row: any) => Pick(row, projection)) : rows
 ...
 */
const RunBatch = async (entries: any, run: any, describe?: any) => {
