...
const Size = (value: any) => {
    ...
    catch(e: any){ return Infinity }   // ciclo/valor não serializável: trata como grande
}
...
const StripHeavyFields = (value: any): any => {
...
const FindLargestList = (data: any) => {
    let best: { holder: any, key: any, list: any[], size: number } | undefined
    const consider = (holder: any, key: any, list: any[]) => {
...
const FitList = (build: any, list: any, limit: any) => {
    let low = 0
 ...
 */
const GuardResponseSize = (data: any, { limit = MAX_RESPONSE_CHARS, toolName }: any = {}) => {
    ...
    if(largest){
        ...
        const apply = (slice: any) => {
            if(!largest.holder) return slice
            largest.holder[field as string] = slice
            return current
        }
...
const _annotate = (data: any, { steps, originalSize, finalSize, toolName }: any) => {
    const notice = {
        truncated: true,
