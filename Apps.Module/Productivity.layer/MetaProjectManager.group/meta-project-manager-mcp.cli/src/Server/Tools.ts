...
const S = {
    str:  (description: any) => ({ type: "string", description }),
    num:  (description: any) => ({ type: "number", description }),
    bool: (description: any) => ({ type: "boolean", description }),
    enum: (values: any, description: any) => ({ type: "string", enum: values, description })
}
const Obj = (properties: any, required?: any) => ({ type: "object", properties, ...(required ? { required } : {}), additionalProperties: false })
...
const { INSTRUCTIONS } = require("./Instructions") as { INSTRUCTIONS: string }
const { MutationResult, ListEnvelope, ITEM_LIST_FIELDS, PROJECT_LIST_FIELDS, RunBatch, Pick } = require("./Envelopes") as {
    MutationResult: (data: any, view: any, entity: string) => any,
    ListEnvelope: (opts: any) => any,
    ITEM_LIST_FIELDS: string[],
    PROJECT_LIST_FIELDS: string[],
    RunBatch: (entries: any, run: any, describe?: any) => Promise<any>,
    Pick: (row: any, fields: string[]) => any
}

const BuildTools = ({ store, actor }: any) => {

    // Anexa o ator do agente ao payload de uma mutação.
    const A = (payload: any) => ({ ...payload, actor })

    // Erro de domínio para a camada MCP (formatado como { ok:false, code, ... }).
    const McpError = (code: any, message: any, details?: any) => Object.assign(new Error(message), { code, details })
    ...
    const PendingFeedbackForItem = async (itemRef: any) => {
        try {
    ...
    const ItemMutationResult = async (data: any, view: any) => {
    ...
    const Written = (entity: any) => (data: any, view?: any) => MutationResult(data, view, entity)
    const VIEW_FIELD_FOR = (entity: any, summaryHint: any) => ({
    ...
    const CreateItemFromInput = async (i: any) => {
    ...
    const AssertNotDeliveryConclusion = async (itemRef: any, status: any) => {
        if(!DONE_LIKE.has(String(status))) return
        let item
        try { item = await store.ResolveItem(itemRef) } catch(e: any){ return }
    ...
    const GatedAction = async ({ actionName = "delete", type, ref, run, waitApproval = true, approvalTimeoutSeconds }: any) => {
    ...
    const GatedDelete = (args: any) => GatedAction({ ...args, actionName: "delete" })
    const GatedCreate = (args: any) => GatedAction({ ...args, actionName: "create" })
    ...
    const DeleteSchema = (targetKey: any, targetDesc: any) => Obj({
    ...
    const DeleteDesc = (alvo: any) => `Remove (SOFT delete) ${alvo}. AÇÃO DESTRUTIVA sob gate: cria um pedido e AGUARDA aprovação humana (a interface mostra O QUE será removido e QUEM pediu). Aprovado → executa e retorna o resultado; rejeitado → { ok:false, code:"REJECTED_BY_HUMAN" }. NÃO tente burlar o gate.`
    ...
    const TOOLS = [
        // ───────────── Estruturar o projeto (projeto/board: GATE de aprovação humana) ─────────────
        {
            ...
            handler: (i: any) => GatedCreate({
                type: "project", ref: i.slug || i.name,
                waitApproval: i.waitApproval, approvalTimeoutSeconds: i.approvalTimeoutSeconds,
                run: (actor: any) => store.CreateProject({ name: i.name, slug: i.slug, shortDescription: i.shortDescription, description: i.description, keyPrefix: i.keyPrefix, status: i.status, repositoryUrl: i.repositoryUrl, localPath: i.localPath, actor })
            })
        },
        {
            name: "create_board",
            ...
            handler: (i: any) => GatedCreate({
                ...
                waitApproval: i.waitApproval, approvalTimeoutSeconds: i.approvalTimeoutSeconds,
                run: (actor: any) => store.CreateBoard({ project: i.project, name: i.name, shortDescription: i.shortDescription, description: i.description, type: i.type, setDefault: i.setDefault, actor })
            })
        },
        {
            ...
            handler: async (i: any) => Written("milestone")(await store.CreateMilestone(A({ project: i.project, name: i.name, shortDescription: i.shortDescription, description: i.description, targetDate: i.targetDate })), i.view)
        },
        {
            name: "create_sprint",
            ...
            handler: async (i: any) => Written("sprint")(await store.CreateSprint(A({ project: i.project, name: i.name, shortDescription: i.shortDescription, goal: i.goal, startDate: i.startDate, endDate: i.endDate })), i.view)
        },
        {
            ...
            handler: async (i: any) => Written("milestoneLink")(await store.LinkMilestones(A({ milestone: i.milestone, relation: i.relation, target: i.target })), i.view)
        },
        {
            ...
            handler: (i: any) => store.UnlinkMilestones(A({ milestone: i.milestone, relation: i.relation, target: i.target }))
        },
        ...
        // contexto E manutenível pelo agente. Criar/editar é LIVRE; excluir é gated.
        {
            ...
            handler: async (i: any) => {
                ...
                const rows = all.slice(offset, offset + limit).map((page: any) => ({
                    ...page,
        ...
        },
        {
            ...
            handler: (i: any) => store.GetDocPage({ docPage: i.docPage })
        },
        {
            ...
            handler: async (i: any) => Written("docPage")(await store.CreateDocPage(A({ project: i.project, parentId: i.parentId, title: i.title, icon: i.icon, body: i.body })), i.view)
        },
        {
            ...
            handler: async (i: any) => Written("docPage")(await store.UpdateDocPage(A({ docPage: i.docPage, title: i.title, icon: i.icon, body: i.body })), i.view)
        },
        {
            name: "move_doc_page",
            ...
            handler: (i: any) => store.MoveDocPage(A({ docPage: i.docPage, parentId: i.parentId, order: i.order }))
        },
        ...
        // Criar/editar é LIVRE; excluir é gated.
        {
            ...
            handler: (i: any) => store.ListRisks({ project: i.project, item: i.item })
        },
        {
            name: "get_risk",
            ...
            handler: (i: any) => store.GetRisk({ risk: i.risk })
        },
        {
            name: "link_risk_item",
            ...
            handler: async (i: any) => Written("riskLink")(await store.LinkRiskItem(A({ risk: i.risk, item: i.item, relation: i.relation, note: i.note })), i.view)
        },
        {
            ...
            handler: (i: any) => store.UnlinkRiskItem(A({ risk: i.risk, item: i.item, relation: i.relation }))
        },
        {
            name: "create_risk",
        ...
        },
        {
            ...
            handler: (i: any) => store.UpdateRisk(A({ risk: i.risk, title: i.title, description: i.description, probability: i.probability, impact: i.impact, status: i.status, category: i.category, mitigation: i.mitigation, contingency: i.contingency, ownerUserId: i.ownerUserId, milestoneId: i.milestoneId }))
        },
        ...
        // sobe a cada edição. Criar/editar é LIVRE; excluir é gated.
        {
            ...
            handler: (i: any) => store.ListPlanningDocs({ project: i.project })
        },
        {
            ...
            handler: (i: any) => store.GetPlanningDoc({ planningDoc: i.planningDoc })
        },
        {
        ...
        // ───────────── Remover (SOFT delete — GATE destrutivo + espera) ─────────────
        {
            ...
            handler: (i: any) => GatedDelete({ type: "project", ref: i.project, waitApproval: i.waitApproval, approvalTimeoutSeconds: i.approvalTimeoutSeconds,
                run: (a: any) => store.DeleteProject({ project: i.project, actor: a }) })
        },
        {
            name: "delete_board",
            description: DeleteDesc("um board"),
            inputSchema: DeleteSchema("board", "Board (id)"),
            handler: (i: any) => GatedDelete({ type: "board", ref: i.board, waitApproval: i.waitApproval, approvalTimeoutSeconds: i.approvalTimeoutSeconds,
                run: (a: any) => store.DeleteBoard({ board: i.board, actor: a }) })
        },
        {
            name: "delete_item",
            description: DeleteDesc("um item de trabalho"),
            inputSchema: DeleteSchema("item", "Item (id|key, ex.: MPM-42)"),
            handler: (i: any) => GatedDelete({ type: "item", ref: i.item, waitApproval: i.waitApproval, approvalTimeoutSeconds: i.approvalTimeoutSeconds,
                run: (a: any) => store.DeleteItem({ item: i.item, actor: a }) })
        },
        {
            ...
            handler: (i: any) => GatedDelete({ type: "doc-page", ref: i.docPage, waitApproval: i.waitApproval, approvalTimeoutSeconds: i.approvalTimeoutSeconds,
                run: (a: any) => store.DeleteDocPage({ docPage: i.docPage, actor: a }) })
        },
        {
            name: "delete_risk",
            description: DeleteDesc("um risco do registro de riscos"),
            inputSchema: DeleteSchema("risk", "Id do risco"),
            handler: (i: any) => GatedDelete({ type: "risk", ref: i.risk, waitApproval: i.waitApproval, approvalTimeoutSeconds: i.approvalTimeoutSeconds,
                run: (a: any) => store.DeleteRisk({ risk: i.risk, actor: a }) })
        },
        {
            ...
            handler: (i: any) => GatedDelete({ type: "planning-doc", ref: i.planningDoc, waitApproval: i.waitApproval, approvalTimeoutSeconds: i.approvalTimeoutSeconds,
                run: (a: any) => store.DeletePlanningDoc({ planningDoc: i.planningDoc, actor: a }) })
        },
        {
            ...
            inputSchema: Obj({ docPage: S.str("Id da página de documentação") }, ["docPage"]),
            handler: (i: any) => store.ListDocPageAttachments({ docPage: i.docPage })
        },
        {
            ...
            handler: (i: any) => store.AddDocPageLinkAttachment(A({ docPage: i.docPage, url: i.url, name: i.name, description: i.description }))
        },
        {
            ...
            handler: (i: any) => store.AddDocPageFileAttachment(A({ docPage: i.docPage, filePath: i.filePath, name: i.name, description: i.description }))
        },
        ...
        // ou remove algo passa pelo gate e BLOQUEIA até a decisão humana.
        {
            ...
            handler: (i: any) => GatedAction({
                actionName: "update", type: "project", ref: i.project,
                waitApproval: i.waitApproval, approvalTimeoutSeconds: i.approvalTimeoutSeconds,
                run: (actor: any) => store.UpdateProject({
        ...
        },
        {
            ...
            handler: (i: any) => GatedAction({
                ...
                waitApproval: i.waitApproval, approvalTimeoutSeconds: i.approvalTimeoutSeconds,
                run: (actor: any) => store.ArchiveProject({ project: i.project, actor })
            })
        },
        {
            ...
            handler: (i: any) => GatedAction({
                ...
                waitApproval: i.waitApproval, approvalTimeoutSeconds: i.approvalTimeoutSeconds,
                run: (actor: any) => store.RestoreProject({ project: i.project, actor })
            })
        },
        {
            name: "close_project",
            ...
            handler: async (i: any) => {
                ...
                return {
                    ...
                    ...(remainingIdeas.length ? {
                        archivedWithIdeas: remainingIdeas.map((idea: any) => ({ key: idea.key, title: idea.title })),
        ...
        },
        {
            ...
            handler: (i: any) => store.GetBoard({ board: i.board })
        },
        {
            name: "update_board",
            ...
            handler: (i: any) => store.UpdateBoard(A({ board: i.board, name: i.name, shortDescription: i.shortDescription, description: i.description }))
        },
        {
            ...
            handler: (i: any) => GatedAction({
                ...
                waitApproval: i.waitApproval, approvalTimeoutSeconds: i.approvalTimeoutSeconds,
                run: (actor: any) => store.SetDefaultBoard({ board: i.board, actor })
            })
        },
        {
            name: "list_columns",
            ...
            inputSchema: Obj({ board: S.str("Board (id)") }, ["board"]),
            handler: (i: any) => store.ListColumns({ board: i.board })
        },
        {
            name: "add_column",
            ...
            handler: (i: any) => GatedAction({
                ...
                waitApproval: i.waitApproval, approvalTimeoutSeconds: i.approvalTimeoutSeconds,
                run: (actor: any) => store.AddColumn({ board: i.board, name: i.name, statusKey: i.statusKey, color: i.color, wipLimit: i.wipLimit, isDoneColumn: i.isDoneColumn, actor })
            })
        },
        {
            name: "update_column",
            ...
            handler: (i: any) => GatedAction({
                ...
                waitApproval: i.waitApproval, approvalTimeoutSeconds: i.approvalTimeoutSeconds,
                run: (actor: any) => store.UpdateColumn({ column: i.column, name: i.name, statusKey: i.statusKey, color: i.color, wipLimit: i.wipLimit, isDoneColumn: i.isDoneColumn, actor })
            })
        },
        {
            name: "move_column",
            ...
            handler: (i: any) => GatedAction({
                ...
                waitApproval: i.waitApproval, approvalTimeoutSeconds: i.approvalTimeoutSeconds,
                run: (actor: any) => store.MoveColumn({ column: i.column, order: i.order, actor })
            })
        },
        {
            name: "delete_column",
            ...
            handler: (i: any) => GatedDelete({
                type: "column", ref: i.column,
                waitApproval: i.waitApproval, approvalTimeoutSeconds: i.approvalTimeoutSeconds,
                run: (actor: any) => store.DeleteColumn({ column: i.column, actor })
            })
        },
        {
            ...
            handler: async (i: any) => Written("milestone")(await store.UpdateMilestone(A({ milestone: i.milestone, name: i.name, shortDescription: i.shortDescription, description: i.description, targetDate: i.targetDate, status: i.status })), i.view)
        },
        {
            ...
            handler: (i: any) => GatedDelete({
                ...
                waitApproval: i.waitApproval, approvalTimeoutSeconds: i.approvalTimeoutSeconds,
                run: (actor: any) => store.DeleteMilestone({ milestone: i.milestone, actor })
            })
        },
        {
            name: "update_sprint",
            ...
            handler: (i: any) => store.UpdateSprint(A({ sprint: i.sprint, name: i.name, goal: i.goal, startDate: i.startDate, endDate: i.endDate, status: i.status }))
        },
        {
            name: "delete_sprint",
            ...
            handler: (i: any) => GatedDelete({
                ...
                waitApproval: i.waitApproval, approvalTimeoutSeconds: i.approvalTimeoutSeconds,
                run: (actor: any) => store.DeleteSprint({ sprint: i.sprint, actor })
            })
        },
        ...
        // ───────────── Detalhe do item: checklist, critérios, vínculos, tipo ─────────────
        {
            ...
            handler: (i: any) => store.AddChecklistItem(A({ item: i.item, text: i.text }))
        },
        {
            ...
            handler: (i: any) => store.UpdateChecklistItem({ checklistItem: i.checklistItem, text: i.text, done: i.done })
        },
        {
            ...
            handler: (i: any) => GatedDelete({
                ...
                waitApproval: i.waitApproval, approvalTimeoutSeconds: i.approvalTimeoutSeconds,
                run: (actor: any) => store.RemoveChecklistItem({ checklistItem: i.checklistItem, actor })
            })
        },
        {
            ...
            handler: async (i: any) => {
                if(Array.isArray(i.texts) && i.texts.length)
                    return RunBatch(
                        i.texts,
                        (text: any) => store.AddAcceptanceCriteria({ item: i.item, text }),
                        (criteria: any) => ({ id: criteria.id, text: criteria.text })
                    )
        ...
        },
        {
            ...
            handler: (i: any) => store.UpdateAcceptanceCriteria({ criteria: i.criteria, updates: i.updates, text: i.text, met: i.met })
        },
        {
            ...
            handler: (i: any) => GatedDelete({
                ...
                waitApproval: i.waitApproval, approvalTimeoutSeconds: i.approvalTimeoutSeconds,
                run: (actor: any) => store.RemoveAcceptanceCriteria({ criteria: i.criteria, actor })
            })
        },
        {
            name: "unlink_item",
            ...
            handler: (i: any) => store.UnlinkItem(A({ item: i.item, relation: i.relation, target: i.target }))
        },
        {
            name: "convert_item",
            ...
            handler: (i: any) => store.ConvertItem(A({ item: i.item, type: i.type }))
        },
        {
            name: "convert_idea",
            ...
            handler: (i: any) => store.ConvertIdea(A({ item: i.item, type: i.type, title: i.title, parent: i.parent }))
        },
        {
            name: "reorder_item",
            ...
            handler: (i: any) => store.ReorderItem(A({ item: i.item, order: i.order }))
        },
        ...
        // ───────────── Executar (itens — LIVRE, sem gate) ─────────────
        {
            ...
            handler: (i: any) => CreateItemFromInput(i)
        },
        {
            name: "create_items",
            ...
            handler: async (i: any) => {
                ...
                const out = await RunBatch(
                    ...
                    (created: any) => ({ key: created.key, id: created.id })
                )
                return out
            }
        },
        {
            ...
            handler: (i: any) => CreateItemFromInput({
        ...
        },
        {
            ...
            handler: async (i: any) => {
                ...
                const result = await GatedAction({
                    ...
                    waitApproval: i.waitApproval, approvalTimeoutSeconds: i.approvalTimeoutSeconds,
                    run: (actor: any) => store.SetStatus({ item: i.item, status: i.status, actor })
                })
        ...
        },
        {
            ...
            handler: async (i: any) => {
                ...
                return GatedAction({
                    ...
                    waitApproval: i.waitApproval, approvalTimeoutSeconds: i.approvalTimeoutSeconds,
                    run: (actor: any) => store.SetStatusBatch({ items: i.items, status: i.status, actor })
                })
            }
        },
        {
            ...
            handler: (i: any) => store.Assign(A({ item: i.item, user: i.user }))
        },
        {
            ...
            handler: (i: any) => store.MoveToBoard(A({ item: i.item, board: i.board, status: i.status }))
        },
        {
            name: "block_item",
            ...
            handler: (i: any) => store.SetBlocked(A({ item: i.item, reason: i.reason }))
        },
        {
            name: "link_item",
            ...
            handler: async (i: any) => Written("link")(await store.LinkItem(A({ item: i.item, relation: i.relation, target: i.target })), i.view)
        },
        {
            name: "link_items",
            ...
            handler: (i: any) => RunBatch(
                i.links,
                (entry: any) => store.LinkItem(A({ item: entry.item, relation: entry.relation, target: entry.target })),
                (link: any) => ({ id: link.id, relation: link.relation })
            )
        },
        {
        ...
        // ───────────── Interagir ─────────────
        {
            ...
            handler: (i: any) => store.AddComment(A({ item: i.item, body: i.body, format: i.format }))
        },
        {
            name: "list_comments",
            ...
            handler: (i: any) => store.ListComments({ item: i.item })
        },
        {
            ...
            handler: (i: any) => store.AddLinkAttachment(A({ item: i.item, url: i.url, name: i.name, description: i.description }))
        },
        {
            ...
            handler: (i: any) => store.AddFileAttachment(A({ item: i.item, filePath: i.filePath, name: i.name, description: i.description }))
        },
        ...
        },
        {
            ...
            inputSchema: Obj({ project: S.str("Projeto (id|slug|key)") }, ["project"]),
            handler: (i: any) => store.GetProject({ project: i.project })
        },
        {
        ...
        },
        {
            ...
            handler: async (i: any) => {
                ...
                return { horizons: out, totals: Object.fromEntries(Object.entries<any>(buckets).map(([k, v]) => [k, v.length])) }
            }
        },
        {
            name: "project_flow",
            ...
            inputSchema: Obj({ project: S.str("Projeto (id|slug|key)") }, ["project"]),
            handler: (i: any) => store.ProjectFlow({ project: i.project })
        },
        ...
        },
        {
            ...
            inputSchema: Obj({ project: S.str("Projeto (id|slug|key)") }, ["project"]),
            handler: (i: any) => store.ListMilestones({ project: i.project })
        },
        {
            name: "list_sprints",
            description: "Lista os sprints do projeto (com progresso) — contexto de execução em curso.",
            inputSchema: Obj({ project: S.str("Projeto (id|slug|key)") }, ["project"]),
            handler: (i: any) => store.ListSprints({ project: i.project })
        },
        {
            name: "report_blocked",
        ...
        },
        {
            ...
            handler: async (i: any) => {
                ...
                return { items: rows.map((row: any) => Pick(row, fields)), total: rows.length }
            }
        },
        {
            name: "list_labels",
            ...
            inputSchema: Obj({ project: S.str("Projeto (id|slug|key)") }, ["project"]),
            handler: (i: any) => store.ListProjectLabels({ project: i.project })
        },
        {
            name: "list_areas",
            ...
            inputSchema: Obj({ project: S.str("Projeto (id|slug|key)") }, ["project"]),
            handler: (i: any) => store.ListProjectAreas({ project: i.project })
        },
        ...
        // NÃO digite o nome de cabeça: liste e use o `ref` que voltar.
        {
            ...
            handler: (i: any) => store.ListEcosystemPackages({
        ...
        },
        {
            ...
            inputSchema: Obj({ item: S.str("Item (id|key)") }, ["item"]),
            handler: (i: any) => store.ListItemPackages({ item: i.item })
        },
        {
            ...
            handler: (i: any) => store.SetItemPackages(A({ item: i.item, packages: i.packages }))
        },
        {
            ...
            handler: (i: any) => store.AddItemPackage(A({ item: i.item, package: i.package, role: i.role, note: i.note }))
        },
        {
            ...
            handler: (i: any) => store.RemoveItemPackage(A({ item: i.item, package: i.package }))
        },
        ...
        // exclusivo e tem prazo) antes de trabalhar, e feche com resolve_feedback.
        {
            ...
            handler: (i: any) => store.ListFeedback({
        ...
        },
        {
            ...
            handler: (i: any) => store.GetFeedback({ feedback: i.feedback })
        },
        {
            name: "claim_feedback",
            ...
            handler: (i: any) => store.ClaimFeedback(A({ feedback: i.feedback, ttlSeconds: i.ttlSeconds }))
        },
        {
            name: "release_feedback",
            description: "Devolve para a fila um feedback que você havia pegado mas não vai resolver agora.",
            inputSchema: Obj({ feedback: S.str("Feedback (id)") }, ["feedback"]),
            handler: (i: any) => store.ReleaseFeedback(A({ feedback: i.feedback }))
        },
        {
            ...
            handler: (i: any) => store.ResolveFeedback(A({ feedback: i.feedback, note: i.note }))
        },
        ...
        },
        {
            ...
            handler: (i: any) => store.GetAuditEvent({ event: i.event })
        },
        {
            ...
            handler: (i: any) => store.AddActivityNote(A({ text: i.text, project: i.project, board: i.board, sprint: i.sprint, milestone: i.milestone, item: i.item, source: "mcp" }))
        },
        {
            ...
            handler: (i: any) => store.ListActivityNotes({ project: i.project, board: i.board, sprint: i.sprint, milestone: i.milestone, item: i.item, from: i.from, to: i.to, limit: i.limit, actor })
        },
        {
            ...
            handler: (i: any) => store.GetActivityContext({ project: i.project, board: i.board, sprint: i.sprint, milestone: i.milestone, item: i.item, limit: i.limit, fullText: i.fullText, noteBodyChars: i.noteBodyChars, actor })
        },
        ...
        // olhando o que foi feito, não o nome da ação.
        {
            ...
            handler: async (a: any) => store.SubmitDelivery(A(a))
        },
        {
            name: "get_delivery",
            ...
            handler: async ({ delivery, view }: any) => store.GetDelivery({ delivery, view })
        },
        {
            ...
            // `ListEnvelope` recebe UM objeto; a chamada antiga passava a lista como
            // 1º argumento posicional, e lá dentro `rows` saía undefined —
            // `items.length` estourava. A checagem de tipo revelou a chamada quebrada.
            handler: async ({ project, item, status, fields, limit, offset }: any) =>
                ListEnvelope({ rows: await store.ListDeliveries({ project, item, status, limit, offset }), limit, offset, fields })
        },
        {
            name: "amend_delivery",
            ...
            handler: async (a: any) => Written("delivery")(await store.AmendDelivery(A(a)))
        },
        {
            ...
            handler: async (a: any) => Written("delivery")(await store.WithdrawDelivery(A(a)))
        },
        {
            ...
            handler: async (a: any) => store.RecollectEvidence(A(a))
        },
        {
            ...
            handler: async (a: any) => store.AddDeliveryNote(A(a))
        },
        ...
        // ── Revisão (papel de REVISOR) ────────────────────────────────────────
        {
            ...
            handler: async (a: any) => store.NextReview(A(a))
        },
        {
            ...
            handler: async (a: any) => store.ListPendingAiReviews(A(a))
        },
        {
            name: "claim_review",
            ...
            handler: async (a: any) => store.ClaimReview(A(a))
        },
        {
            name: "release_review",
            description: "Devolve à fila uma revisão que você reivindicou e não vai concluir.",
            inputSchema: Obj({ delivery: S.str("Entrega (id|key)") }, ["delivery"]),
            handler: async (a: any) => Written("delivery")(await store.ReleaseReview(A(a)))
        },
        {
            name: "submit_review",
            ...
            handler: async (a: any) => store.SubmitReview(A(a))
        },
        ...
        // ── Mandato (o escopo em que você anda sozinho) ───────────────────────
        {
            ...
            handler: async (a: any) => (await store.CurrentMandate(A(a))) || { mandate: undefined, message: "Você não tem mandato ativo aqui: trabalhe normalmente, pedindo o que precisar." }
        },
        {
            ...
            handler: async (a: any) => store.RequestMandateExtension(A(a))
        },
        {
            ...
            handler: async (a: any) => store.WaitForMandate(a)
        },
        ...
        // ── Plano (você propõe, o humano aceita UMA vez) ──────────────────────
        {
            ...
            handler: async (a: any) => store.ProposePlan(A(a))
        },
        {
            name: "get_plan",
            ...
            handler: async ({ plan }: any) => store.GetPlan({ plan })
        },
        {
            name: "list_plans",
            ...
            handler: async (a: any) => store.ListPlans(a)
        },
        {
            name: "revise_plan",
            ...
            handler: async (a: any) => store.RevisePlan(A(a))
        },
        {
            name: "wait_for_plan",
            ...
            handler: async (a: any) => store.WaitForPlan(a)
        },
        ...
        // ── Papel ─────────────────────────────────────────────────────────────
        {
            ...
            handler: async (a: any) => store.DeclareRole(A(a))
        },
        ...
        },
        {
            ...
            inputSchema: Obj({ project: S.str("Projeto (id|slug|key)") }, ["project"]),
            handler: (i: any) => store.SequenceView({ project: i.project })
        },
        {
            name: "project_pulse",
            ...
            handler: (i: any) => store.ProjectPulse({ project: i.project, limit: i.limit })
        },
        {
            ...
            handler: (i: any) => store.ReportProgress(A({ item: i.item, project: i.project, note: i.note, phase: i.phase }))
        },
        {
            name: "claim_item",
            ...
            handler: (i: any) => store.ClaimItem(A({ item: i.item, minutes: i.minutes, packages: i.packages }))
        },
        {
            name: "next_task",
            ...
            handler: (i: any) => store.NextTask(A({ project: i.project, minutes: i.minutes, area: i.area, label: i.label, type: i.type }))
        },
        {
            name: "who_is_here",
            ...
            handler: (i: any) => store.WhoIsHere(A({ project: i.project, includeGone: i.includeGone }))
        },
        {
            ...
            handler: (i: any) => store.SendSessionMessage(A({ toSession: i.toSession, item: i.item, project: i.project, body: i.body }))
        },
        {
            name: "agent_inbox",
        ...
        },
        {
            ...
            handler: (i: any) => store.UpdateSessionFocus(A({ objective: i.objective, currentFocus: i.currentFocus, sessionName: i.sessionName }))
        },
        {
            ...
            handler: (i: any) => store.RecordEnvironmentAction(A({ project: i.project, action: i.action, target: i.target, note: i.note }))
        },
        {
            ...
            handler: (i: any) => store.ListEnvironmentActions(A({ project: i.project, limit: i.limit }))
        },
        {
            name: "end_session",
            ...
            handler: (i: any) => store.EndSession(A({ note: i.note }))
        },
        {
            name: "release_item",
            ...
            inputSchema: Obj({ item: S.str("Item (id|key)") }, ["item"]),
            handler: (i: any) => store.ReleaseItem(A({ item: i.item }))
        },
        {
            name: "complete_epic",
            ...
            handler: async (i: any) => {
                ...
                return GatedAction({
                    ...
                    waitApproval: i.waitApproval, approvalTimeoutSeconds: i.approvalTimeoutSeconds,
                    run: (actor: any) => store.CompleteEpic({ item: i.epic, status: i.status || "done", actor })
                })
            }
        },
        {
            ...
            handler: (i: any) => store.DeclareSession(A({
    ...
    const SessionGate = async (toolName: any) => {
        try {
    ...
    return TOOLS.map((tool) => IsReadOnlyTool(tool.name)
        ? tool
        : { ...tool, handler: async (input: any) => { await SessionGate(tool.name); return tool.handler(input) } })
}
