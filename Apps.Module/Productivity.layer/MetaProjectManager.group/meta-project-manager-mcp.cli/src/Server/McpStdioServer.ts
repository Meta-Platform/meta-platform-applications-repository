...
const { GuardResponseSize } = require("./ResponseGuard") as { GuardResponseSize: (data: any, opts?: any) => { data: any, degraded: boolean } }

const PROTOCOL_VERSION = "2024-11-05"
const SUPPORTED_PROTOCOLS = ["2025-06-18", "2025-03-26", "2024-11-05"]

const CreateMcpStdioServer = ({ name, version, tools, logger, instructions, collectNotices }: any) => {
    ...
    const Send = (message: any) => {
        ...
        catch(e: any){ logger.error("falha ao escrever no stdout:", e && e.message) }
    }

    const Reply = (id: any, result: any) => Send({ jsonrpc: "2.0", id, result })
    const ReplyError = (id: any, code: any, message: any, data?: any) =>
        Send({ jsonrpc: "2.0", id, error: { code, message, ...(data ? { data } : {}) } })

    const HandleInitialize = (id: any, params: any) => {
    ...
    const HandleToolsCall = async (id: any, params: any) => {
    ...
    const Start = () => {
        ...
        process.stdin.on("data", (chunk) => {
            ...
            while((index = buffer.indexOf("\n")) >= 0){
                ...
                catch(e: any){ logger.error("mensagem JSON-RPC inválida ignorada:", line.slice(0, 200)); continue }
