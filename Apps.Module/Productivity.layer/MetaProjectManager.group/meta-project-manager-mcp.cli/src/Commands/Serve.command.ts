const { InitStore } = require("../Utils/runtime") as { InitStore: (opts: any) => Promise<any> }
const { BuildAgentActor } = require("../Utils/actor") as { BuildAgentActor: (opts?: any) => any }
const { BuildTools } = require("../Server/Tools") as { BuildTools: (opts: any) => any[] }
const { CreateMcpStdioServer } = require("../Server/McpStdioServer") as { CreateMcpStdioServer: (opts: any) => { Start: () => void, Dispatch: (msg: any) => Promise<any> } }
const { INSTRUCTIONS } = require("../Server/Instructions") as { INSTRUCTIONS: string }
const { CreateLogger } = require("../Utils/logger") as { CreateLogger: (scope?: string) => { info: (...a: any[]) => void, warn: (...a: any[]) => void, error: (...a: any[]) => void } }
...
const ServeCommand = async ({ args = {}, startupParams, params }: any) => {
    ...
    process.on("unhandledRejection", (e: any) => logger.error("unhandledRejection:", e && (e.stack || e.message) || e))
