...
const GitSnapshot = (cwd: string) => {
    const run = (cmd: string) => { try { return execSync(cmd, { cwd, stdio: ["ignore", "pipe", "ignore"] }).toString().trim() } catch(e: any){ return undefined } }
    return {
...
const BuildAgentActor = ({ args = {}, env = process.env }: any = {}) => {
