...
const TOPICS: Topic[] = [
    {
        ...
        body: <>
            ...
            <ul className="mpm-guide-list">
                ...
                <li><T>Planejamento</T> — <T>Entrega</T> (um alvo com data), <T>Sprint</T> (janela de tempo) e <T>Horizonte</T> (ideia/agora/próximo…) organizam <i>quando</i> cada item acontece.</li>
            </ul>
            <p>Um fluxo típico: crie um <T>Projeto</T> → ele ganha um <T>Board</T> com colunas → você joga <T>itens</T> no board e os move de coluna conforme avançam → agrupa itens em <T>Entregas</T>/<T>Sprints</T> e acompanha no <T>Planejamento</T>.</p>
    ...
    },
    {
        ...
        body: <>
            <p>Um <T>Projeto</T> é o espaço de um produto/sistema/iniciativa. Dentro dele vivem boards, itens, entregas, sprints, comentários e anexos.</p>
            <ul className="mpm-guide-list">
    ...
    {
        key: "backlog-inbox", label: "Backlog, Ideias & Horizonte", icon: "clipboard list",
        lead: "Onde as ideias e o trabalho não-agendado ficam antes de entrar no fluxo.",
        body: <>
            <H3>Ideias</H3>
            <p>A tela <T>Ideias</T> é onde você anota <T>ideias cruas</T> sem decidir nada — capturar rápido antes de esquecer. Depois você faz a triagem: vira tarefa/história, vai para o backlog ou é descartada.</p>
            <Callout icon="terminal">No jargão técnico (e na API/MCP) isso se chama <M>inbox</M>: horizonte <M>inbox</M>, clareza <M>idea</M>.</Callout>
            <H3>Backlog</H3>
            ...
            <Table head={["Horizonte", "Significa"]} rows={[
                [<M>inbox</M>, "Ideia recém-capturada, sem triagem (aparece na tela Ideias)"],
    ...
    {
        key: "milestone", label: "Entrega (milestone)", icon: "flag",
        lead: "Um alvo com data. Agrupa itens rumo a um marco (ex.: um release).",
        body: <>
            <p>Uma <T>Entrega</T> é um <T>alvo</T> com uma <T>data-alvo</T>. Você associa itens a ela e acompanha o <T>progresso</T> (itens concluídos / total) rumo àquela data.</p>
            <Callout icon="terminal">No jargão técnico (e na API/MCP) isso se chama <M>milestone</M> — às vezes traduzido como “marco” ou “release”. É a mesma coisa.</Callout>
            <ul className="mpm-guide-list">
            ...
            <Callout icon="road">As entregas aparecem no <T>Planejamento</T> ordenadas por data-alvo, cada uma com sua barra de progresso.</Callout>
        </>
    },
    {
        ...
        body: <>
            ...
            <Callout icon="info circle"><T>Entrega × Sprint:</T> a entrega é uma <b>meta com data</b> (pode levar vários sprints); o sprint é uma <b>caixa de tempo</b> curta e repetível. Um item pode estar em um sprint <i>e</i> contar para uma entrega.</Callout>
        </>
    },
    {
        key: "roadmap", label: "Planejamento (roadmap)", icon: "road",
        lead: "A visão do plano no tempo: por data (entregas) ou por horizonte (agora/próximo/depois).",
        body: <>
            <p>O <T>Planejamento</T> é a visão do plano do projeto no tempo. Ele tem dois modos:</p>
            <ul className="mpm-guide-list">
                <li><T>Por data</T> — a linha do tempo das <T>entregas</T> ordenada pela data-alvo, com progresso de cada uma; os <T>sprints</T> aparecem logo abaixo.</li>
                <li><T>Por horizonte</T> — os itens agrupados em colunas <M>agora / próximo / depois / talvez</M> (e ideias), para enxergar prioridade sem depender de datas.</li>
            </ul>
            <Callout icon="terminal">No jargão técnico essa visão se chama <M>roadmap</M>.</Callout>
    ...
    },
    {
        ...
        body: <>
            <Table head={["Atributo", "Valores", "Para quê"]} rows={[
                ...
                [<T>Planejamento</T>, "Visão do plano no tempo, por data ou por horizonte (roadmap, no jargão técnico)."],
                ...
                [<T>Entrega</T>, "Alvo com data; agrupa itens rumo a um marco (milestone, no jargão técnico)."],
    ...
    },
    {
        ...
        body: <>
            <Table head={["Termo", "Definição curta"]} rows={[
                ...
                [<T>Feedback</T>, "Correção que você pede a um agente num campo específico (botão direito). Ele pega, aplica e resolve."],
                [<T>Horizonte</T>, "Proximidade de execução: inbox/now/next/later/maybe."],
                [<T>Ideias</T>, "Caixa de ideias cruas para triagem posterior (inbox, no jargão técnico)."],
...
const GlossaryPage = () => {
    ...
    return <AppShell active="glossary"
        breadcrumb={[{ label: "Manual & Glossário" }]}
        title="Manual & Glossário"
        subtitle="O que é cada objeto do gerenciador — Projeto, Board, Item, Entrega, Sprint, Planejamento… — e como usar."
        actions={
