...
const BoardsStore = (ctx: any) => {
    ...
    const _setDefaultBoard = async (projectInstance: any, boardId: any) => {
    ...
    const _withDefaultFlag = (board: any, project: any) => ({
        ...Serialize(board),
        isDefault: !!project && project.defaultBoardId === board.id
    })

    const _createDefaultColumns = async (boardId: any) => {
        let order = 0
    ...
    const CreateBoard = async ({ project, name, shortDescription, description, type = "kanban", withDefaultColumns = true, setDefault, actor }: any = {}) => {
    ...
    const ListBoards = async ({ project }: any = {}) => {
        const projectInstance = await store.ResolveProject(project)
        const rows = await Board.findAll({ where: { projectId: projectInstance.id, deletedAt: null }, order: [["createdAt", "ASC"]] })
        return rows.map((b: any) => _withDefaultFlag(b, projectInstance))
    }

    const GetBoard = async ({ board }: any = {}) => {
    ...
    const UpdateBoard = async ({ board, actor, ...fields }: any = {}) => {
        const instance = await ResolveBoard(board)
    ...
    const DuplicateBoard = async ({ board, name, actor }: any = {}) => {
    ...
    const DeleteBoard = async ({ board, actor }: any = {}) => {
        const instance = await ResolveBoard(board)
        await store.AssertProjectWritable({ project: instance.projectId })
    ...
    const SetDefaultBoard = async ({ board, actor }: any = {}) => {
        const instance = await ResolveBoard(board)
        await store.AssertProjectWritable({ project: instance.projectId })
    ...
    const _projectIdOfColumn = async (column: any) => {
        const board = await ResolveBoard(column.boardId)
        return board.projectId
    }

    const ListColumns = async ({ board }: any = {}) => {
        const instance = await ResolveBoard(board)
        const rows = await BoardColumn.findAll({ where: { boardId: instance.id }, order: [["order", "ASC"]] })
        return SerializeMany(rows)
    }

    const AddColumn = async ({ board, name, statusKey, color, wipLimit, isDoneColumn = false, actor }: any = {}) => {
        const instance = await ResolveBoard(board)
        await store.AssertProjectWritable({ project: instance.projectId })
    ...
    const UpdateColumn = async ({ column, actor, ...fields }: any = {}) => {
    ...
    const MoveColumn = async ({ column, order, actor }: any = {}) => {
        const instance = await BoardColumn.findOne({ where: { id: column } })
        if(!instance) throw new DomainError("NOT_FOUND", `Coluna "${column}" não encontrada.`, { ref: column })
        await store.AssertProjectWritable({ project: await _projectIdOfColumn(instance) })
        await store.GateAgentAction({
        ...
        const columns = (await BoardColumn.findAll({ where: { boardId: instance.boardId }, order: [["order", "ASC"], ["createdAt", "ASC"]] }))
            .filter((col: any) => col.id !== instance.id)
    ...
    const DeleteColumn = async ({ column, actor }: any = {}) => {
        const instance = await BoardColumn.findOne({ where: { id: column } })
        if(!instance) throw new DomainError("NOT_FOUND", `Coluna "${column}" não encontrada.`, { ref: column })
        await store.AssertProjectWritable({ project: await _projectIdOfColumn(instance) })
        await store.GateAgentAction({
