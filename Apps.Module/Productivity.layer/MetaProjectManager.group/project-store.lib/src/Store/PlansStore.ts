 ...
 */
const _normalizarNo = (node: any, indice: any) => {
    ...
    const minusculo = (v: any) => (typeof v === "string" && v ? v.toLowerCase() : v)
    const normalizado = {
        ...node,
    ...
    const checar = (valor: any, permitidos: any, campo: any) => {
...
const PlansStore = (ctx: any) => {
     ...
     */
    const ProposePlan = async ({ project, title, shortDescription, rationale, risks, nodes = [], submit = true, actor }: any = {}) => {
        ...
        for(const row of criados){
            ...
            const ids = refs.map((r: any) => porRef[String(r).replace(/^@/, "")] || r).filter(Boolean)
    ...
    const ListPlans = async ({ project, status }: any = {}) => {
    ...
    const RevisePlan = async ({ plan, node, updates = {}, actor }: any = {}) => {
        const row = await ResolvePlan(plan)
     ...
     */
    const AcceptPlan = async ({ plan, createSprint = true, createMandate = true, actor }: any = {}) => {
        const row = await ResolvePlan(plan)
    ...
    const RejectPlan = async ({ plan, reason, actor }: any = {}) => {
        const row = await ResolvePlan(plan)
    ...
    const WaitForPlan = async ({ plan, timeoutSeconds = 0, intervalMs = 2000 }: any = {}) => {
        const row = await ResolvePlan(plan)
    ...
    const DeclareRole = async ({ role, project, actor }: any = {}) => {
    ...
    const GrantRole = async ({ agent, session, role, project, note, actor }: any = {}) => {
        const row = await AgentRoleAssignment.create({
            id: NewId(),
    ...
    const RevokeRole = async ({ assignment, actor }: any = {}) => {
    ...
    const ListRoles = async ({ project, agent, session, role, includeRevoked }: any = {}) => {
        const where: any = {}
        if(project) where.projectId = (await store.ResolveProject(project)).id
