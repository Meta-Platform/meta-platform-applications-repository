...
const PlanningDocsStore = (ctx: any) => {
    ...
    const ResolvePlanningDoc = async (ref: any) => {
    ...
    const _assertStatus = (status: any) => {
    ...
    const ListPlanningDocs = async ({ project }: any = {}) => {
    ...
    const GetPlanningDoc = async ({ planningDoc }: any = {}) => Serialize(await ResolvePlanningDoc(planningDoc))

    const CreatePlanningDoc = async ({ project, title, milestoneId, status, actor, ...sections }: any = {}) => {
    ...
    const UpdatePlanningDoc = async ({ planningDoc, actor, ...fields }: any = {}) => {
    ...
    const DeletePlanningDoc = async ({ planningDoc, actor }: any = {}) => {
        const instance = await ResolvePlanningDoc(planningDoc)
        await store.AssertProjectWritable({ project: instance.projectId })
