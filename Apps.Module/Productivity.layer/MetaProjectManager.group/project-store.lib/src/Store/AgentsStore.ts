...
const AgentsStore = (ctx: any) => {
    ...
    const CreateAgent = async ({ provider = "other", owner, name, displayName, handle, defaultModel, externalAgentId, description, actor }: any = {}) => {
    ...
    const ResolveAgent = async (ref: any) => {
    ...
    const GetAgent = async ({ agent }: any = {}) => {
    ...
    const ResolveSession = async (ref: any) => {
    ...
    const ListSessions = async ({ agent, status, limit = 200, offset = 0 }: any = {}) => {
        const where: any = {}
    ...
    const GetSession = async ({ session }: any = {}) => Serialize(await ResolveSession(session))
    ...
    const ConfirmSession = async ({ session, provider, model, actor }: any = {}) => {
        const instance = await ResolveSession(session)
        const patch: any = { status: "active", confirmedAt: new Date(), presence: "here", presenceChangedAt: new Date() }
    ...
    const RejectSession = async ({ session, actor }: any = {}) => {
        const instance = await ResolveSession(session)
    ...
    const CloseSession = async ({ session, actor }: any = {}) => {
        const instance = await ResolveSession(session)
    ...
    const _resolveAgentUserForIdentity = async ({ provider = "other", agent, owner }: any) => {
        if(agent){
    ...
    const FindSessionByIdentity = async (identity: any = {}) =>
    ...
    const ResolveOrCreateSessionByIdentity = async (identity: any = {}, action: any) => {
     ...
     */
    const AssertSessionApproved = async ({ actor, action }: any = {}) => {
     ...
     */
    const DeclareSession = async ({ actor, provider, model, objective, currentFocus, sessionName }: any = {}) => {
    ...
    const WaitForSessionDecision = async ({ session, timeoutMs = 0, pollMs = 1000 }: any = {}) => {
    ...
    const IsAgentActor = (actor: any) => !!(actor && actor.session)
    ...
    const _projectModel = async (projectId: any) => {
    ...
    const ForgetProjectModel = (projectId: any) => { _modelCache.delete(projectId) }
    ...
    const GateAgentAction = async ({ actionName, type, targetId, projectId, payload = {}, risk = "normal", reason, actor }: any = {}) => {
    ...
    const RequestCreation = async ({ type, payload = {}, projectId, resumeToken, actor }: any = {}) =>
        RequestApproval({ actionName: "create", type, payload, projectId, risk: "normal", resumeToken, actor })

    const ResolveCreationRequest = async (ref: any) => {
    ...
    const DescribeDeletionImpact = async ({ type, targetId }: any = {}) => {
        if(type === "project"){
    ...
    const _SUBJECT_LOADERS: any = {
        project: async (id: any) => {
        ...
        "acceptance-criteria": async (id: any) => {
    ...
    const _loadWorkItemSubject = async (id: any) => {
    ...
    const _IMPLICIT_CHANGES: any = {
        "archive:project":   (current: any) => [{ field: "status", from: current.status, to: "archived" }],
        "restore:project":   (current: any) => [{ field: "status", from: current.status, to: "active" }],
        "set-default:board": (current: any) => [{ field: "isDefault", from: !!current.isDefault, to: true }]
    }

    const _describeChanges = ({ actionName, type, payload = {}, current = {} }: any) => {
        const implicit = _IMPLICIT_CHANGES[`${actionName}:${type}`]
        if(implicit) return implicit(current)
        const map = _CHANGE_MAP[`${actionName}:${type}`]
        if(!map) return []
        const changes = []
        for(const [payloadKey, targetField] of Object.entries(map) as [string, any][]){
    ...
    const DescribeApprovalSubject = async ({ actionName = "create", type, targetId, payload = {} }: any = {}) => {
        ...
        if(actionName === "set-status-batch" && Array.isArray(payload.detail)){
            ...
            subject.batch = payload.detail.map((d: any) => ({
                key: d.key, title: d.title, from: d.fromStatus, to: payload.status,
                unmetCriteria: d.unmetCriteria || []
            }))
            subject.unmetCriteria = payload.detail.flatMap((d: any) => d.unmetCriteria || [])
            subject.changes = []
        }
        ...
        if(actionName === "complete-epic"){
            const children: any[] = []
            const visit = async (parentId: any, depth: any) => {
                if(depth > 6) return
    ...
    const _describeWho = (session: any, req: any) => session ? {
    ...
    const APPROVAL_EXECUTORS: any = {
        "create:project":   ({ payload, actor }: any) => store.CreateProject({ ...payload, actor }),
        "create:board":     ({ payload, actor }: any) => store.CreateBoard({ ...payload, actor }),
        "create:milestone": ({ payload, actor }: any) => store.CreateMilestone({ ...payload, actor }),
        "create:sprint":    ({ payload, actor }: any) => store.CreateSprint({ ...payload, actor }),

        "delete:project":   ({ targetId, actor }: any) => store.DeleteProject({ project: targetId, actor }),
        "delete:board":     ({ targetId, actor }: any) => store.DeleteBoard({ board: targetId, actor }),
        "delete:item":      ({ targetId, actor }: any) => store.DeleteItem({ item: targetId, actor }),
        "delete:work-item": ({ targetId, actor }: any) => store.DeleteItem({ item: targetId, actor }),
        "delete:milestone": ({ targetId, actor }: any) => store.DeleteMilestone({ milestone: targetId, actor }),
        "delete:sprint":    ({ targetId, actor }: any) => store.DeleteSprint({ sprint: targetId, actor }),
        "delete:column":    ({ targetId, actor }: any) => store.DeleteColumn({ column: targetId, actor }),
        "delete:checklist-item":      ({ targetId, actor }: any) => store.RemoveChecklistItem({ checklistItem: targetId, actor }),
        "delete:acceptance-criteria": ({ targetId, actor }: any) => store.RemoveAcceptanceCriteria({ criteria: targetId, actor }),

        // Reescrita de texto do projeto e mudança de ciclo de vida.
        "update:project":   ({ payload, targetId, actor }: any) => store.UpdateProject({ ...payload, project: targetId, actor }),
        "archive:project":  ({ targetId, actor }: any) => store.ArchiveProject({ project: targetId, actor }),
        "restore:project":  ({ targetId, actor }: any) => store.RestoreProject({ project: targetId, actor }),

        // Iniciar/concluir tarefa: aprovada, reexecuta o SetStatus (actor sem
        // session → não redispara o gate).
        "set-status:work-item": ({ payload, actor }: any) => store.SetStatus({ item: payload.item, status: payload.status, actor }),

        // O MESMO, para o conjunto: uma decisão humana move todos os itens do
        // pedido (MPMX3-8).
        "set-status-batch:work-item": ({ payload, actor }: any) =>
        ...
        "complete-epic:work-item": ({ payload, actor }: any) => store.CompleteEpic({ item: payload.item, status: payload.status, actor }),

        // Estrutura do board: colunas e board padrão.
        "create:column":    ({ payload, actor }: any) => store.AddColumn({ ...payload, actor }),
        "update:column":    ({ payload, targetId, actor }: any) => store.UpdateColumn({ ...payload, column: targetId, actor }),
        "move:column":      ({ payload, targetId, actor }: any) => store.MoveColumn({ column: targetId, order: payload.order, actor }),
        "set-default:board": ({ targetId, actor }: any) => store.SetDefaultBoard({ board: targetId, actor }),
        ...
        "close:project":    ({ payload, targetId, actor }: any) =>
        ...
        "grant:mandate":    ({ payload, targetId, actor }: any) =>
        ...
        // numa decisão só.
        "accept:plan":      ({ payload, targetId, actor }: any) =>
    ...
    const _resolveDecider = async (actor: any = {}) => {
     ...
     */
    const _assertNotStale = async (req: any, payload: any) => {
    ...
    const ApproveRequest = async ({ request, actor }: any = {}) => {
        ...
        const decider: any = await _resolveDecider(actor)
     ...
     */
    const DecideRequests = async ({ requests = [], decision = "approve", reason, actor }: any = {}) => {
    ...
    const RejectRequest = async ({ request, reason, actor }: any = {}) => {
        const req = await ResolveCreationRequest(request)
        if(req.status !== "pending")
            throw new DomainError("VALIDATION_ERROR", `Pedido já ${req.status}.`, { status: req.status })
        const decider: any = await _resolveDecider(actor)
    ...
    const WaitForApproval = async ({ request, timeoutMs = 0, pollMs = 1000 }: any = {}) => {
        const started = Date.now()
        for(;;){
    ...
    const DescribeCreationRequest = async ({ request }: any = {}) => {
        const req = await ResolveCreationRequest(request)
    ...
    const ListCreationRequests = async ({ type, actionName, status = "pending", agent, session, projectId, limit = 200, offset = 0 }: any = {}) => {
        const where: any = {}
        ...
        if(session) where.agentSessionId = session
            ...
            where.agentSessionId = sessions.map((s: any) => s.id)
        }
    ...
    const AgentGatePolicy = async ({ project }: any = {}) => {
        let model = "legacy"
        if(project){
