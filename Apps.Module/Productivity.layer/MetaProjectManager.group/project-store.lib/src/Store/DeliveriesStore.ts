...
const DeliveriesStore = (ctx: any) => {
     ...
     */
    const _registrarRevisaoHumana = async ({ row, decision, reason, actor }: any) => {
    ...
    const ResolveDelivery = async (ref: any) => {
     ...
     */
    const SubmitDelivery = async ({ item, summary, title, shortDescription, verifyCommand, actor }: any = {}) => {
        ...
        return {
            ...
            warnings: colheita && colheita.gaps && colheita.gaps.length
                ? colheita.gaps.filter((g: any) => g.severity === "blocking").map((g: any) => g.title || g.ref)
                : undefined
        }
    }

    const GetDelivery = async ({ delivery, view }: any = {}) => {
        ...
        const PESO: any = { blocking: 0, warning: 1, info: 2 }
        evidence.sort((a: any, b: any) => (PESO[a.severity] ?? 3) - (PESO[b.severity] ?? 3))
        ...
        if(view === "review"){
            ...
            base.blockingGaps = base.evidence.filter((e: any) => e.severity === "blocking")
        }
        return base
    }

    const ListDeliveries = async ({ project, item, status, aiReviewState, awaitingHuman, limit, offset }: any = {}) => {
    ...
    const AmendDelivery = async ({ delivery, summary, title, actor }: any = {}) => {
        const row = await ResolveDelivery(delivery)
    ...
    const WithdrawDelivery = async ({ delivery, reason, actor }: any = {}) => {
        const row = await ResolveDelivery(delivery)
     ...
     */
    const ReturnDelivery = async ({ delivery, reason, reviewerType = "human", actor }: any = {}) => {
        const row = await ResolveDelivery(delivery)
     ...
     */
    const AcceptDelivery = async ({ delivery, note, actor }: any = {}) => {
        const row = await ResolveDelivery(delivery)
    ...
    const RecollectEvidence = async ({ delivery, actor }: any = {}) => {
        const row = await ResolveDelivery(delivery)
        const item = await WorkItem.findOne({ where: { id: row.workItemId } })
    ...
    const AddDeliveryNote = async ({ delivery, title, body, actor }: any = {}) => {
        const row = await ResolveDelivery(delivery)
