...
const InferType = (name: any, mimeType: any) => {
...
const DocAttachmentsStore = (ctx: any) => {
    ...
    const _persist = async ({ projectId, attachmentId, buffer }: any) => {
    ...
    const _writeMeta = async (dir: any, meta: any) =>
        fs.writeFile(path.join(dir, "metadata.json"), JSON.stringify(meta, null, 2))

    const _store = async ({ docPage, name, buffer, mimeType, description, type, actor }: any) => {
    ...
    const AddDocPageFileAttachment = async ({ docPage, filePath, name, description, type, actor }: any = {}) => {
        ...
        catch(e: any){ throw new DomainError("VALIDATION_ERROR", `Não foi possível ler o arquivo: ${filePath}.`, { field: "file" }) }
    ...
    const AddDocPageBufferAttachment = async ({ docPage, name, buffer, base64, mimeType, description, type, actor }: any = {}) => {
    ...
    const AddDocPageLinkAttachment = async ({ docPage, url, name, description, actor }: any = {}) => {
    ...
    const ListDocPageAttachments = async ({ docPage }: any = {}) => {
        const page = await store.ResolveDocPage(docPage)
    ...
    const ResolveDocPageAttachment = async (ref: any) => {
    ...
    const GetDocPageAttachment = async ({ attachment }: any = {}) => Serialize(await ResolveDocPageAttachment(attachment))

    // Retorna { buffer, name, mimeType } para download/preview (não p/ links).
    const ReadDocPageAttachment = async ({ attachment }: any = {}) => {
    ...
    const RemoveDocPageAttachment = async ({ attachment, actor }: any = {}) => {
        const att = await ResolveDocPageAttachment(attachment)
    ...
    const _RemoveDocPageAttachmentsOfPages = async (docPageIds: any) => {
