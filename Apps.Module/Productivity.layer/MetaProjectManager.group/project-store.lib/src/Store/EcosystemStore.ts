...
// MCP e GUI.
const EcosystemStore = ({ models, writeAudit, emit, store, config }: any) => {
    const { EcosystemPackage, WorkItemPackage } = models

    const MAX_DEPTH = 4     // Module/layer/group/pacote

    const _expandHome = (target: any) =>
    ...
    const _isPackageDir = (absolute: any) => {
        try { return fs.statSync(path.join(absolute, "metadata", "package.json")).isFile() }
        catch(_: any){ return false }
    }

    const _scanRepository = (repository: any) => {
        const found: any[] = []
        const walk = (absolute: any, relativeParts: any) => {
            if(relativeParts.length > MAX_DEPTH) return
            let entries
            try { entries = fs.readdirSync(absolute, { withFileTypes: true }) } catch(_: any){ return }
    ...
    const IndexEcosystemPackages = async ({ actor }: any = {}) => {
    ...
    const EcosystemIndexStatus = async () => {
        ...
        catch(e: any){ declarationError = { code: e.code || "ERROR", message: e.message } }
    ...
    const GetEcosystemPackage = async ({ package: ref }: any = {}) => {
    ...
    const ListItemPackages = async ({ item }: any = {}) => {
    ...
    const _resolvePackage = async (reference: any) => {
        ...
        if(matches.length > 1)
            throw new DomainError("VALIDATION_ERROR",
                ...
                { field: "package", candidates: matches.map((m: any) => m.ref) })
        throw new DomainError("NOT_FOUND",
            `Pacote "${reference}" não está no catálogo. Rode a reindexação ou confira o nome.`,
            { ref: reference })
    }

    const AddItemPackage = async ({ item, package: reference, role = "touched", note, actor }: any = {}) => {
        const instance = await store.ResolveItem(item)
    ...
    const RemoveItemPackage = async ({ item, package: reference, actor }: any = {}) => {
        const instance = await store.ResolveItem(item)
        await store.AssertProjectWritable({ project: instance.projectId })
    ...
    const SetItemPackages = async ({ item, packages = [], actor }: any = {}) => {
        const instance = await store.ResolveItem(item)
        await store.AssertProjectWritable({ project: instance.projectId })
        const wanted = packages.map((entry: any) =>
