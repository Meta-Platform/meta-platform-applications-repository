...
const FeedbackStore = ({ models, writeAudit, emit, store }: any) => {
    ...
    // não valeria nada.
    const _agentIdentity = async (actor: any = {}) => {
    ...
    const _isAgent = (actor: any = {}) => !!(actor.session || actor.actorSessionId)
    ...
    const CreateFeedback = async ({
        ...
        if(workItem){
            const where: any = [field ? `no campo **${fieldLabel || field}**` : null, `(feedback \`${row.id}\`)`]
                .filter(Boolean).join(" ")
            const commentBody = [
            ...
            catch(e: any){ /* o comentário é um espelho: nunca derruba a criação do feedback */ }
        }
    ...
    const GetFeedback = async ({ feedback }: any = {}) => _serialize(await ResolveFeedbackRow(feedback))
    ...
    const ClaimFeedback = async ({ feedback, ttlSeconds = CLAIM_TTL_SECONDS, actor = {} }: any = {}) => {
    ...
    const ReleaseFeedback = async ({ feedback, actor = {} }: any = {}) => {
        const row = await ResolveFeedbackRow(feedback)
    ...
    const ResolveFeedback = async ({ feedback, note, actor = {} }: any = {}) => {
        const row = await ResolveFeedbackRow(feedback)
    ...
    const DismissFeedback = async ({ feedback, reason, actor = {} }: any = {}) => {
        const row = await ResolveFeedbackRow(feedback)
    ...
    const ReopenFeedback = async ({ feedback, actor = {} }: any = {}) => {
        const row = await ResolveFeedbackRow(feedback)
        await row.update({
    ...
    const CountOpenFeedback = async ({ project }: any = {}) => {
