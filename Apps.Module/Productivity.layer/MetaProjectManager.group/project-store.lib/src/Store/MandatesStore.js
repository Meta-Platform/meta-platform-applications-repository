...
const MandatesStore = (ctx) => {
    ...
    const ActivateMandate = async ({ mandate, actor } = {}) => {
    ...
    const ListMandates = async ({ project, status, agent, session, activeOnly } = {}) => {
     ...
     */
    const EvaluateMandate = async ({ mandate } = {}) => {
        const row = await ResolveMandate(mandate)
     ...
     */
    const AssertMandate = async ({ project, item, actor } = {}) => {
        ...
        if(mandate.status === "exhausted" || mandate.status === "revoked"){
            ...
            throw new DomainError("MANDATE_EXHAUSTED",
                `O mandato "${mandate.title}" parou: ${_explain(mandate.stopReason)}.`,
                {
                    ...
                    pendingDeliveries: (pendentes || []).map((d) => d.key),
                    whatNow: [
    ...
    })[stopReason] || "uma condição de parada foi atingida"

    // ── Contadores (chamados pelo DeliveriesStore) ───────────────────────────

    const CountDeliveryOnMandate = async ({ delivery, session } = {}) => {
    ...
    const CountReturnOnMandate = async ({ delivery } = {}) => {
        if(!delivery || !delivery.mandateId) return
        const row = await AgentMandate.findOne({ where: { id: delivery.mandateId } })
        if(!row) return
        await row.update({
    ...
    const CountAcceptOnMandate = async ({ delivery } = {}) => {
        if(!delivery || !delivery.mandateId) return
        const row = await AgentMandate.findOne({ where: { id: delivery.mandateId } })
        if(!row) return
        await row.update({
    ...
    const ExtendMandate = async ({ mandate, maxDeliveries, maxUnreviewedDeliveries, maxConsecutiveReturns, expiresAt, note, actor } = {}) => {
        const row = await ResolveMandate(mandate)
        const patch = { status: "active", stopReason: null, stoppedAt: null }
    ...
    const RequestMandateExtension = async ({ mandate, reason, actor } = {}) => {
        const row = await ResolveMandate(mandate)
        await store.GateAgentAction({
    ...
    const RevokeMandate = async ({ mandate, reason, actor } = {}) => {
        const row = await ResolveMandate(mandate)
        await row.update({
    ...
    const WaitForMandate = async ({ mandate, timeoutSeconds = 0, intervalMs = 2000 } = {}) => {
        const row = await ResolveMandate(mandate)
