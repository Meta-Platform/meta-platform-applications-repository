...
const AnalyticsStore = (ctx: any) => {
    ...
    const _dayKey = (d: any) => new Date(d).toISOString().slice(0, 10)
    const _startOfDay = (dayKey: any) => new Date(`${dayKey}T00:00:00.000Z`).getTime()
    const _addDays = (dayKey: any, n: any) => _dayKey(_startOfDay(dayKey) + n * 86400000)
    ...
    const _columnsFor = async (projectInstance: any) => {
        let cols = []
        ...
        if(board){
            ...
            cols = rows.map((c: any) => ({ statusKey: c.statusKey, name: c.name, color: c.color, isDoneColumn: !!c.isDoneColumn }))
        }
        if(cols.length === 0)
            cols = DEFAULT_COLUMNS.map((c: any) => ({ statusKey: c.statusKey, name: c.name, color: c.color, isDoneColumn: c.isDoneColumn }))
        return cols
    }
    ...
    const _timeline = (item: any, events: any) => {
        const evs = events
            .filter((e: any) => e.entityId === item.id)
    ...
    const _statusAt = (points: any, boundary: any) => {
    ...
    const ProjectFlow = async ({ project }: any = {}) => {
        ...
        const timelines = items.map((it: any) => ({ item: it, points: _timeline(it, events) }))

        // Janela: do dia do item mais antigo até hoje (limitada a MAX_DAYS).
        const firstT = Math.min(...timelines.map((t: any) => t.points[0].t))
        ...
        const known = new Set(columns.map((c: any) => c.statusKey))
        const extra = new Set()
        ...
        const allColumns = columns.concat([...extra].map((s: any) => ({ statusKey: s, name: s, color: "#94A3B8", isDoneColumn: DONE.has(s) })))

        const doneNow = items.filter((i: any) => DONE.has(i.statusKey)).length
    ...
    const ItemTimeline = async ({ project, items: preloaded }: any = {}) => {
        const projectInstance = await store.ResolveProject(project)
