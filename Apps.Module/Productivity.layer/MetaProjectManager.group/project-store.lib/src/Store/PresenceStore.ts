...
const PresenceStore = (ctx: any) => {
     ...
     */
    const _sessionOf = async (actor: any = {}, { create = false }: any = {}) => {
     ...
     */
    const CreateNotice = async ({ project, kind = "message", toSession, body, metadata = {}, fromSessionId }: any = {}) => {
     ...
     */
    const SendSessionMessage = async ({ toSession, item, project, body, actor = {} }: any = {}) => {
     ...
     */
    const CollectNotices = async ({ actor = {}, limit = 10, markRead = true }: any = {}) => {
        ...
        if(markRead && notices.length){
            const directed = rows.filter((r: any) => r.toSessionId === session.id)
    ...
    const ListNotices = async ({ session, project, kind, unreadOnly = false, limit = 30 }: any = {}) => {
        const where: any = {}
     ...
     */
    const TouchSession = async ({ actor = {}, action, project }: any = {}) => {
        const session = await _sessionOf(actor, { create: true })
        if(!session) return undefined
        const now = new Date()
        const wasGone = session.presence === "gone"
        const patch: any = { lastActivityAt: now, actionCount: (session.actionCount || 0) + 1 }
    ...
    const AnnounceArrival = async ({ session, project, returning = false }: any = {}) => {
     ...
     */
    const EndSession = async ({ note, actor = {} }: any = {}) => {
        const session = await _sessionOf(actor, { create: true })
        if(!session)
     ...
     */
    const WarnAboutCompany = async ({ session }: any = {}) => {
        ...
        for(const other of others){
            ...
            const what = held.length ? ` em ${held.map((i: any) => i.key).join(", ")}`
        ...
        return CreateNotice({
            ...
            metadata: { sessions: others.map((o: any) => o.id) }
        })
    }
     ...
     */
    const WhoIsHere = async ({ project, includeGone = false, actor = {} }: any = {}) => {
        await SweepPresence()
        const projectId = project ? (await store.ResolveProject(project)).id : undefined
        ...
        for(const session of sessions){
            ...
            const progressWhere: any = { authorSessionId: session.id, kind: "progress", deletedAt: null }
            ...
            out.push({
                ...
                claimedItems: claimed.map((i: any) => ({ key: i.key, title: i.title, statusKey: i.statusKey, until: i.claimExpiresAt })),
     ...
     */
    const UpdateSessionFocus = async ({ objective, currentFocus, sessionName, actor = {} }: any = {}) => {
        const session = await _sessionOf(actor, { create: true })
        if(!session)
     ...
     */
    const RecordEnvironmentAction = async ({ project, action = "other", target, note, actor = {} }: any = {}) => {
    ...
    const ListEnvironmentActions = async ({ project, limit = 30, actor }: any = {}) =>
