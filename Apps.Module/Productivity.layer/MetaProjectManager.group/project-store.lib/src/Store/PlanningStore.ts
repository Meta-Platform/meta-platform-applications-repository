...
const PlanningStore = (ctx: any) => {
    ...
    const _progress = async (field: any, id: any) => {
        ...
        const effort: any = { total: 0, done: 0, remaining: 0, estimated: 0, unestimated: 0, byBucket: {} }
        const confidence: any = { unset: 0 }
        ...
        return {
            ...
            derivedStatus:
                ...
                : items.some((i: any) => !DONE.has(i.statusKey) && !["backlog", "ready"].includes(i.statusKey)) ? "active"
                : done > 0                                                ? "active"
                : "planned"
        }
    }
    ...
    const _resolveByIdOrName = async (Model: any, ref: any, { projectId, label, field }: any = {}) => {
        if(!ref) throw new DomainError("VALIDATION_ERROR", `Referência de ${label} é obrigatória.`, { field })
        const byId: any = await Model.findOne({ where: { id: ref, deletedAt: null } })
        if(byId) return byId
        const where: any = { name: ref, deletedAt: null }
        ...
        if(matches.length > 1)
            throw new DomainError("VALIDATION_ERROR",
                ...
                { field, candidates: matches.map((m: any) => ({ id: m.id, name: m.name, projectId: m.projectId })) })
        throw new DomainError("NOT_FOUND", `${label[0].toUpperCase()}${label.slice(1)} "${ref}" não encontrado.`, { ref })
    }

    const ResolveMilestone = async (ref: any, projectId?: any) =>
        _resolveByIdOrName(Milestone, ref, { projectId, label: "milestone", field: "milestone" })

    const CreateMilestone = async ({ project, name, shortDescription, description, targetDate, status = "planning", actor }: any = {}) => {
    ...
    const _dependencyEdge = (link: any) => link.relation === "blocks"
    ...
    const _assertNoDependencyCycle = async (projectId: any, start: any, forbidden: any, extraEdge: any) => {
    ...
    const LinkMilestones = async ({ milestone, relation = "depends", target, actor }: any = {}) => {
    ...
    const UnlinkMilestones = async ({ milestone, relation, target, actor }: any = {}) => {
        const source = await ResolveMilestone(milestone)
        await store.AssertProjectWritable({ project: source.projectId })
        const targetInstance = await ResolveMilestone(target)
        const where: any = { sourceMilestoneId: source.id, targetMilestoneId: targetInstance.id }
    ...
    const _milestoneRelations = async (projectId: any) => {
        const [rows, links] = await Promise.all([
            Milestone.findAll({ where: { projectId, deletedAt: null } }),
            _projectMilestoneLinks(projectId)
        ])
        const byId: any = {}
        rows.forEach((m: any) => { byId[m.id] = m })
        const brief = (id: any) => byId[id] ? { id, name: byId[id].name, status: byId[id].status, targetDate: byId[id].targetDate } : { id, missing: true }

        const relations: any = {}
        rows.forEach((m: any) => { relations[m.id] = { dependsOn: [], blocks: [] } })
        ...
        for(const id of Object.keys(relations)){
            const pending = relations[id].dependsOn.filter((d: any) => !d.missing && d.status !== "released" && d.status !== "archived")
            relations[id].dependenciesMet = pending.length === 0
            relations[id].pendingDependencies = pending.map((d: any) => d.name)
        }
        return relations
    }

    const ListMilestones = async ({ project, includeProgress = true }: any = {}) => {
        const projectInstance = await store.ResolveProject(project)
        const rows = await Milestone.findAll({ where: { projectId: projectInstance.id, deletedAt: null }, order: [["targetDate", "ASC"], ["order", "ASC"]] })
        const relations: any = await _milestoneRelations(projectInstance.id)
        const out = []
        for(const m of rows)
            out.push({
                ...Serialize(m),
    ...
    const GetMilestone = async ({ milestone }: any = {}) => {
        const m = await ResolveMilestone(milestone)
        const relations: any = await _milestoneRelations(m.projectId)
        return {
            ...Serialize(m),
    ...
    const UpdateMilestone = async ({ milestone, actor, ...fields }: any = {}) => {
        const m = await ResolveMilestone(milestone)
    ...
    const DeleteMilestone = async ({ milestone, actor }: any = {}) => {
        const m = await ResolveMilestone(milestone)
        await store.AssertProjectWritable({ project: m.projectId })
    ...
    const RoadmapByHorizon = async ({ project }: any = {}) => {
        const projectInstance = await store.ResolveProject(project)
        const items = await WorkItem.findAll({ where: { projectId: projectInstance.id, deletedAt: null }, order: [["order", "ASC"]] })
        const buckets: any = { inbox: [], now: [], next: [], later: [], maybe: [], archived: [], unassigned: [] }
    ...
    // ---------------- Sprints ----------------
    const ResolveSprint = async (ref: any, projectId?: any) =>
        _resolveByIdOrName(Sprint, ref, { projectId, label: "sprint", field: "sprint" })

    const CreateSprint = async ({ project, name, shortDescription, goal, startDate, endDate, status = "planned", actor }: any = {}) => {
    ...
    const ListSprints = async ({ project, includeProgress = true }: any = {}) => {
        const projectInstance = await store.ResolveProject(project)
    ...
    const DeleteSprint = async ({ sprint, actor }: any = {}) => {
        const s = await ResolveSprint(sprint)
        await store.AssertProjectWritable({ project: s.projectId })
        await store.GateAgentAction({
    ...
    const AssignItemPlanning = async ({ item, milestone, sprint, actor }: any = {}) => {
