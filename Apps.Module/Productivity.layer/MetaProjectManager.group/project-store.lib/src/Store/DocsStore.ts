...
const DocsStore = (ctx: any) => {
    ...
    const _assertNoCycle = async (pageId: any, parentId: any) => {
        let cursor = parentId
        const seen = new Set()
        while(cursor){
    ...
    const ListDocPages = async ({ project }: any = {}) => {
    ...
    const GetDocPage = async ({ docPage }: any = {}) => Serialize(await ResolveDocPage(docPage))

    const CreateDocPage = async ({ project, parentId, title, icon, body, actor }: any = {}) => {
    ...
    const UpdateDocPage = async ({ docPage, actor, ...fields }: any = {}) => {
    ...
    const MoveDocPage = async ({ docPage, parentId, order, actor }: any = {}) => {
        const instance = await ResolveDocPage(docPage)
        await store.AssertProjectWritable({ project: instance.projectId })
        const patch: any = {}
    ...
    const _collectSubtree = async (rootId: any) => {
        const ids = [rootId]
        let frontier = [rootId]
        let guard = 0
        while(frontier.length > 0 && guard < 10000){
            ...
            frontier = children.map((c: any) => c.id).filter((id: any) => !ids.includes(id))
            ids.push(...frontier)
        }
        return ids
    }

    const DeleteDocPage = async ({ docPage, actor }: any = {}) => {
        const instance = await ResolveDocPage(docPage)
        await store.AssertProjectWritable({ project: instance.projectId })
