...
const UsersStore = (ctx: any) => {
    ...
    const CreateUser = async ({ type = "human", name, displayName, handle, email, avatarUrl, actor }: any = {}) => {
    ...
    const ListUsers = async ({ type, status, limit = 200, offset = 0 }: any = {}) => {
    ...
    const GetUser = async ({ user }: any = {}) => Serialize(await ResolveUser(user))

    const UpdateUser = async ({ user, actor, ...fields }: any = {}) => {
    ...
    const ArchiveUser = async ({ user, actor, force = false }: any = {}) => {
        const instance = await ResolveUser(user)
    ...
    const _parsePermissions = (user: any) => {
        if(!user || !user.permissionsJson) return []
        try { const p = JSON.parse(user.permissionsJson); return Array.isArray(p) ? p : [] }
        catch(e: any){ return [] }
    }

    const GetUserPermissions = async ({ user }: any = {}) => _parsePermissions(await ResolveUser(user))

    const SetUserPermissions = async ({ user, permissions = [], actor }: any = {}) => {
        const instance = await ResolveUser(user)
        const invalid = permissions.filter((p: any) => !PERMISSIONS.includes(p))
        if(invalid.length)
    ...
    const HasPermission = async ({ user, permission }: any = {}) => {
    ...
    const _resolveAgentUserId = async (actor: any = {}) => {
    ...
    const AssertGlobalActivityAccess = async ({ actor, permission = "activity:read:all_projects" }: any = {}) => {
