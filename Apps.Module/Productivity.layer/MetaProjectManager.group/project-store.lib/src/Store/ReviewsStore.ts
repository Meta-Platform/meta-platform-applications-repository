...
const ReviewsStore = (ctx: any) => {
     ...
     */
    const AssertReviewerEligible = async ({ delivery, sessionId, actor }: any = {}) => {
     ...
     */
    const ListPendingAiReviews = async ({ project, limit, actor }: any = {}) => {
        await SweepStaleAiReviews({ project })
        const session = await _sessionOf(actor)
        const where: any = { status: "ai-review", aiReviewState: { [Op.in]: ["pending", "claimed"] }, deletedAt: null }
        ...
        const disponiveis = rows.filter((r: any) =>
        ...
        return {
            ...
            hiddenBecauseYours: rows.filter((r: any) => session && r.executedBySessionId === session.id).length
        }
    }
     ...
     */
    const ClaimReview = async ({ delivery, minutes, actor }: any = {}) => {
    ...
    const ReleaseReview = async ({ delivery, actor }: any = {}) => {
        const row = await store.ResolveDelivery(delivery)
        const session = await _sessionOf(actor)
    ...
    const NextReview = async ({ project, minutes, actor }: any = {}) => {
     ...
     */
    const SubmitReview = async ({ delivery, decision, reason, criteriaVerdict = [], evidenceSeen = [], durationMs, actor }: any = {}) => {
        const row = await store.ResolveDelivery(delivery)
        const session = await _sessionOf(actor)
    ...
    const _toAwaitingHuman = async (row: any, actor?: any) => {
    ...
    const ListDeliveryReviews = async ({ delivery, item, project }: any = {}) => {
        const where: any = {}
     ...
     */
    const SweepStaleAiReviews = async ({ project }: any = {}) => {
        const where: any = { status: "ai-review", deletedAt: null }
        if(project) where.projectId = (await store.ResolveProject(project)).id
    ...
    const EscalateToHuman = async ({ delivery, reason, actor }: any = {}) => {
        const row = await store.ResolveDelivery(delivery)
        await row.update({
            status: "awaiting-human", aiReviewState: "escalated",
     ...
     */
    const ReviewDesk = async ({ project, limit = 50, actor }: any = {}) => {
        await SweepStaleAiReviews({ project })
        const projectId = project ? (await store.ResolveProject(project)).id : undefined
        const where: any = { status: "awaiting-human", deletedAt: null }
