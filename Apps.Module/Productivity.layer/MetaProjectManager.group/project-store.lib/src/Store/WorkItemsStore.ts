const { Op, literal, fn, col, cast, where: whereExpression } = require("sequelize") as any
...
const _assertEnum = (val: any, allowed: any, field: any) => {
    const { DomainError } = require("../Errors")
...
const WorkItemsStore = (ctx: any) => {
    ...
    const _isDoneStatus = async (instance: any, status: any) => {
    ...
    const _countsByItem = async (ids: any) => {
        const att: any = {}, com: any = {}
        ...
        attRows.forEach((r: any) => { att[r.workItemId] = Number(r.n) })
        const comRows = await Comment.findAll({
            where: { workItemId: { [Op.in]: ids }, deletedAt: null },
            attributes: ["workItemId", [fn("COUNT", col("id")), "n"]], group: ["workItemId"], raw: true
        })
        comRows.forEach((r: any) => { com[r.workItemId] = Number(r.n) })
        return { att, com }
    }

    // Resolve item por id ou key (ex.: MPM-42, case-insensitive).
    const ResolveItem = async (ref: any) => {
    ...
    const _resolveUserId = async (ref: any) => {
    ...
    const _assertNoCycle = async (itemId: any, parentId: any) => {
        let cursor = parentId
        const seen = new Set()
        while(cursor){
    ...
    const _vocabularyKey = (value: any) => String(value === null || value === undefined ? "" : value)
        .normalize("NFD").replace(/[̀-ͯ]/g, "")
        .toLowerCase().replace(/[\s_-]+/g, " ").trim()

    // Áreas em uso no projeto, da mais frequente para a menos.
    const ListProjectAreas = async ({ project }: any = {}) => {
    ...
    const ListProjectLabels = async ({ project }: any = {}) => {
        const projectId = (await store.ResolveProject(project)).id
    ...
    const _normalizeArea = async (area: any, projectId: any) => {
    ...
    const _resolveParent = async (parentRef: any, projectId: any) => {
    ...
    const _buildItemWhere = async ({ project, type, status, parent, board, assignee, text, priority, milestone, sprint, horizon, clarityState, effort, confidence, value, area, label, release, package: pkg, includeClaimed, actor }: any = {}) => {
    ...
    const ListItems = async (filters: any = {}) => {
        ...
        const { att, com } = await _countsByItem(list.map((i: any) => i.id))
        list.forEach((i: any) => { i.attachmentCount = att[i.id] || 0; i.commentCount = com[i.id] || 0 })
    ...
    const GetItem = async ({ item }: any = {}) => {
        ...
        const linkIds = [...new Set(links.flatMap((l: any) => [l.sourceItemId, l.targetItemId]))]
        ...
        linkedRows.forEach((r: any) => { linkedById[r.id] = r })
        const serializeLink = (l: any) => {
    ...
    const UpdateItem = async ({ item, actor, ...fields }: any = {}) => {
        const instance = await ResolveItem(item)
    ...
    const _sessionIdOf = async (actor: any) => {
     ...
     */
    const _packageCollisions = async ({ item, sessionId }: any) => {
        if(!store.ListItemPackages) return []
        const mine = await store.ListItemPackages({ item: item.id }).catch(() => [])
        const myNames = mine.map((p: any) => p.ref || p.namespace || p.packageName).filter(Boolean)
        ...
        const others = rows.filter((r: any) => r.claimedBySessionId && r.claimedBySessionId !== sessionId)
    ...
    const ClaimItem = async ({ item, minutes, packages, actor }: any = {}) => {
        const instance = await ResolveItem(item)
        await store.AssertProjectWritable({ project: instance.projectId })
     ...
     * donos.
     */
    const NextTask = async ({ project, minutes, area, label, type, actor }: any = {}) => {
        const sessionId = await _sessionIdOf(actor)
        if(!sessionId)
        ...
        for(const candidate of queue){
            ...
            return {
                ...
                alsoHere: here ? here.sessions.filter((s: any) => !s.isYou).map((s: any) =>
                    `${s.label}${s.claimedItems.length ? ` em ${s.claimedItems.map((i: any) => i.key).join(", ")}` : ""}`) : undefined,
    ...
    const RenewItemClaim = async ({ item, minutes, actor }: any = {}) => {
        const instance = await ResolveItem(item)
        const sessionId = await _sessionIdOf(actor)
        const current = _claimOf(instance)
    ...
    const ReleaseItem = async ({ item, actor }: any = {}) => {
        const instance = await ResolveItem(item)
        const sessionId = await _sessionIdOf(actor)
        const current = _claimOf(instance)
    ...
    const GetItemClaim = async ({ item }: any = {}) => _claimOf(await ResolveItem(item))
     ...
     */
    const CompleteEpic = async ({ item, status = "done", actor }: any = {}) => {
     ...
     */
    const UnmetAcceptanceCriteria = async ({ item }: any = {}) => {
     ...
     */
    const SetExecutionState = async ({ item, executionState, reviewState, extra = {}, actor }: any = {}) => {
        const instance = typeof item === "object" && item.id ? item : await ResolveItem(item)
    ...
    const SetStatus = async ({ item, status, actor }: any = {}) => {
        ...
        const patch: any = { statusKey: status, pendingStatusKey: null, pendingStatusRequestId: null, pendingStatusAt: null }
     ...
     */
    const SetStatusBatch = async ({ items = [], status, actor }: any = {}) => {
        if(!status) throw new DomainError("VALIDATION_ERROR", "Status é obrigatório.", { field: "status" })
    ...
    const Assign = async ({ item, user, actor }: any = {}) => {
        const instance = await ResolveItem(item)
        await store.AssertProjectWritable({ project: instance.projectId })
     ...
     */
    const MoveItemToProject = async ({ item, project, actor }: any = {}) => {
        const instance = await ResolveItem(item)
    ...
    const MoveItem = async ({ item, parent, actor }: any = {}) => {
        const instance = await ResolveItem(item)
        await store.AssertProjectWritable({ project: instance.projectId })
        let parentId = null
    ...
    const MoveToBoard = async ({ item, board, status, actor }: any = {}) => {
        const instance = await ResolveItem(item)
        await store.AssertProjectWritable({ project: instance.projectId })
        const boardInstance = await store.ResolveBoard(board)
        const patch: any = { boardId: boardInstance.id }
    ...
    const ReorderItem = async ({ item, order, actor }: any = {}) => {
        const instance = await ResolveItem(item)
        await store.AssertProjectWritable({ project: instance.projectId })
    ...
    const ConvertItem = async ({ item, type, actor }: any = {}) => {
    ...
    const ConvertIdea = async ({ item, type = "task", title, parent, actor }: any = {}) => {
        if(!WORK_ITEM_TYPES.includes(type)) throw new DomainError("VALIDATION_ERROR", `Tipo inválido: ${type}.`, { field: "type", allowed: WORK_ITEM_TYPES })
    ...
    const SetBlocked = async ({ item, reason, actor }: any = {}) => {
        const instance = await ResolveItem(item)
        await store.AssertProjectWritable({ project: instance.projectId })
    ...
    const LinkItem = async ({ item, relation, target, actor }: any = {}) => {
    ...
    const UnlinkItem = async ({ item, relation, target, actor }: any = {}) => {
        const src = await ResolveItem(item)
        await store.AssertProjectWritable({ project: src.projectId })
        const tgt = await ResolveItem(target)
    ...
    const DeleteItem = async ({ item, actor }: any = {}) => {
        const instance = await ResolveItem(item)
        await store.AssertProjectWritable({ project: instance.projectId })
    ...
    // -------- Checklist --------
    const AddChecklistItem = async ({ item, text, actor }: any = {}) => {
        const instance = await ResolveItem(item)
        await store.AssertProjectWritable({ project: instance.projectId })
    ...
    const UpdateChecklistItem = async ({ checklistItem, text, done }: any = {}) => {
    ...
    const RemoveChecklistItem = async ({ checklistItem, actor }: any = {}) => {
        const row = await WorkItemChecklistItem.findOne({ where: { id: checklistItem } })
        if(!row) throw new DomainError("NOT_FOUND", "Item de checklist não encontrado.", { ref: checklistItem })
        const owner = await ResolveItem(row.workItemId)
        await store.AssertProjectWritable({ project: owner.projectId })
    ...
    // -------- Critérios de aceite --------
    const AddAcceptanceCriteria = async ({ item, text }: any = {}) => {
        const instance = await ResolveItem(item)
        await store.AssertProjectWritable({ project: instance.projectId })
     ...
     */
    const UpdateAcceptanceCriteria = async ({ criteria, updates, text, met }: any = {}) => {
    ...
    const RemoveAcceptanceCriteria = async ({ criteria, actor }: any = {}) => {
