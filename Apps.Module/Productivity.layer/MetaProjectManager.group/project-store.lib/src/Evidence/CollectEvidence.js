 ...
 */
const CollectEvidence = (ctx) => {
    const { models, store, config } = ctx
    const { DeliveryEvidence } = models

    const CollectEvidenceForDelivery = async ({ delivery, item, project } = {}) => {
        const now = new Date()
     ...
     */
    const _quality = ({ evidence, gaps, git }) => {
        const bloqueantes = gaps.filter((g) => g.severity === "blocking")
        const temCommit = evidence.some((e) => e.kind === "commit")
        const verificacao = evidence.find((e) => e.kind === "verification")

        if(bloqueantes.some((g) => g.ref === "repo-nao-declarado")) return "none"
...
const _within = (inicio, orcamento) => Date.now() - inicio < orcamento

const _budgetGap = (nome, now) => ({
    kind: "gap", source: "system", collectorName: nome,
