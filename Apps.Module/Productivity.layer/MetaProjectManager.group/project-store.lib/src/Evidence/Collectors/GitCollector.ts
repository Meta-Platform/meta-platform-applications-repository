 ...
 */
const GitCollector = async ({ delivery, item, project, models, gitLib, now }: any) => {
    const evidence: any[] = []
    const gaps = []
 ...
 * um `.git`).
 */
const _resolveRepository = async ({ item, project, models }: any) => {
    const candidatos = []
...
const _isLocalPath = (v: any) => typeof v === "string" && (v.startsWith("/") || v.startsWith("~"))
...
const _findGitRoot = (start: any) => {
    ...
    } catch(e: any){ /* caminho inválido: simplesmente não serve */ }
    return undefined
}

const _declaredPackages = async ({ item, models }: any) => {
    if(!models || !models.WorkItemPackage) return []
    const rows = await models.WorkItemPackage.findAll({ where: { workItemId: item.id } }).catch(() => [])
    return rows.map((r: any) => r.packageName || r.ref).filter(Boolean)
}
...
const _belongsToDeclared = (filePath: any, declarados: any) =>
    declarados.some((nome: any) => filePath.includes(nome.split("/").pop()))

const _gap = (ref: any, title: any, severity: any, now: any) => ({
