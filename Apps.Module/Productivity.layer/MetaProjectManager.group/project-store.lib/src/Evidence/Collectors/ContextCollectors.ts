...
const CriteriaCollector = async ({ delivery, item, store, now }: any) => {
    const evidence: any[] = []
    const gaps: any[] = []
    ...
    const emAberto = criterios.filter((c: any) => !c.met)
    evidence.push({
        ...
        dataJson: { criteria: criterios.map((c: any) => ({ id: c.id, text: c.text, met: !!c.met })) }
    })

    if(emAberto.length)
        gaps.push(_gap("criterio-em-aberto",
            `${emAberto.length} critério(s) de aceite ainda não atendido(s): ${emAberto.map((c: any) => c.text).slice(0, 3).join(" · ")}${emAberto.length > 3 ? "…" : ""}`,
...
const ActivityCollector = async ({ delivery, item, models, now }: any) => {
    const evidence: any[] = []
...
const PackageCollector = async ({ item, models, now }: any) => {
    const evidence: any[] = []
    ...
    evidence.push({
        ...
        collectedAt: now, occurredAt: now,
        dataJson: { packages: pacotes.map((p: any) => ({ ref: p.ref, role: p.role, path: p.packagePath })) }
    })
    return { evidence, gaps: [] }
}

const _resumo = (texto: any) => String(texto || "").split("\n")[0].slice(0, 120)

const _gap = (ref: any, title: any, severity: any, now: any, collectorName: any) => ({
