...
const _suffixOf = (name: any) => {
    const index = name.lastIndexOf(".")
    return index > 0 ? name.slice(index + 1) : ""
}
const _baseOf = (name: any) => {
    const index = name.lastIndexOf(".")
    return index > 0 ? name.slice(0, index) : name
}

const IsContainer = (name: any) => CONTAINER_SUFFIXES.indexOf(_suffixOf(name)) >= 0

// `relativePath` é relativo à raiz do repositório, com "/" como separador.
// Devolve null quando o caminho não descreve um pacote.
const ParsePackagePath = (relativePath: any) => {
...
const PackageRef = (repositoryName: any, namespace: any) => `${repositoryName}:${namespace}`
