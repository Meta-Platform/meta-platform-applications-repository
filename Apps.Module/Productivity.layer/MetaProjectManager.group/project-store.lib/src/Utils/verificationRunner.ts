 ...
 */
const CreateVerificationRunner = ({ instanceManagerClient }: any = {}) => {
    if(!instanceManagerClient) return undefined

    return async ({ command, args = [], cwd, timeoutMs = 180000 }: any = {}) => {
        ...
        if(!stream || typeof stream.on !== "function"){
            try { stream && stream.close && stream.close() } catch(e: any){ /* nada a fechar */ }
            return { exitCode: null, output: "", durationMs: Date.now() - inicio,
        ...
        return new Promise((resolve) => {
            ...
            const terminar = (resultado: any) => {
                if(finalizado) return
                finalizado = true
                clearTimeout(relogio)
                try { stream.close && stream.close() } catch(e: any){ /* já fechado */ }
            ...
            stream.on("message", (raw: any) => {
                let mensagem
                try { mensagem = JSON.parse(raw) } catch(e: any){ return }
            ...
            stream.on("error", (e: any) => terminar({ exitCode: null, error: String(e && e.message || e) }))
 ...
 */
const BuildVerificationRunner = ({ instanceManagerClientLib, ecosystemDataPath, socketPath, onUnavailable }: any = {}) => {
    const avisar = (motivo: any) => { try { onUnavailable && onUnavailable(motivo) } catch(e: any){ /* nunca atrapalha */ } }
...
const _defaultSocketPath = (ecosystemDataPath: any) => {
