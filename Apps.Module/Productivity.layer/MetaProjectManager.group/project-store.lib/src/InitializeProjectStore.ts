...
const InitializeProjectStore = (options = {}) => {
    ...
    const config: any = typeof options === "string" ? { storage: options } : { ...options }
    ...
    const MigrateAddedColumns = async () => {
        for(const [table, column, type] of ADDED_COLUMNS){
            ...
            catch(e: any){ /* coluna já existe (ou tabela recém-criada pelo sync) — ignora */ }
        }
    }

    // Emissor de eventos realtime; noop se não houver onEvent.
    const emit = (type: any, payload: any) => {
        if(typeof config.onEvent === "function"){
            try { config.onEvent({ type, payload, createdAt: new Date().toISOString() }) } catch(e: any){ /* nunca derruba mutação por causa de listener */ }
        }
    }
    ...
    const writeAudit = async (entry: any) => {
    ...
    const ListActivity = MakeListActivity({
        assertGlobalActivityAccess: (args: any) => store.AssertGlobalActivityAccess(args)
    })

    // Store montado incrementalmente para permitir referências cruzadas
    // (ex.: WorkItems -> ResolveProject/ResolveUser/ResolveBoard).
    const store: any = { models, sequelize, ConnectAndSync }
