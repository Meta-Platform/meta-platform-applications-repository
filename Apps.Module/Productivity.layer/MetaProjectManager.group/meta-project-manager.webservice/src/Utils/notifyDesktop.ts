 ...
 */
const CreateDesktopNotifier = ({ url, appKey = "meta-project-manager", timeoutMs = 1500 }: any = {}) => {
    if(!url) return () => undefined

    const endpoint = new URL(url)

    return ({ title, body, kind, level, dedupeKey, count }: any = {}) => {
        if(!title) return
        ...
        try {
            ...
            req.on("timeout", () => { try { req.destroy() } catch(e: any){ /* já morreu */ } })
            req.write(payload)
            req.end()
        } catch(e: any){ /* nunca derruba a operação que originou o aviso */ }
    }
}
