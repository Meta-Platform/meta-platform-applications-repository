...
const EcosystemController = (params: any) => {
    const ctx = GetContext(params)
    const { store } = ctx

    const ListPackages = async (p: any = {}) => Guard(async () => {
        await ctx.ready
    ...
    const GetPackage = async (arg: any) => Guard(async () => {
        await ctx.ready
    ...
    const IndexPackages = async (p: any = {}) => Guard(async () => {
        await ctx.ready
        return store.IndexEcosystemPackages({ actor: Actor(p) })
    })

    const ListItemPackages = async (arg: any) => Guard(async () => {
        await ctx.ready
        return store.ListItemPackages({ item: idOf(arg, "itemId") })
    })

    const SetItemPackages = async (p: any = {}) => Guard(async () => {
        await ctx.ready
