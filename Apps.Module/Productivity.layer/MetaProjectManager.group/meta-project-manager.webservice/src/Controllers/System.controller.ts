...
const SystemController = (params: any) => {
    const ctx = GetContext(params)
    const { store } = ctx

    const ExportProject = async (arg: any) => Guard(async () => { await ctx.ready; return store.ExportProject({ project: idOf(arg, "projectId") }) })
    const ExportBoard   = async (arg: any) => Guard(async () => { await ctx.ready; return store.ExportBoard({ board: idOf(arg, "boardId") }) })
    const ImportProject = async (p: any = {}) => Guard(async () => { await ctx.ready; return store.ImportProject({ data: p.data, actor: { source: "api" } }) })

    const GetAppState = async (arg: any) => Guard(async () => { await ctx.ready; const value = await store.GetAppState({ key: idOf(arg, "key") }); return { key: idOf(arg, "key"), value: value === undefined ? null : value } })
    const SetAppState = async (p: any = {}) => Guard(async () => { await ctx.ready; return store.SetAppState({ key: p.key, value: p.value }) })
    ...
    const GetEnvironmentInfo = async () => Guard(async () => {
        ...
        let osUser; try { osUser = os.userInfo().username } catch(e: any) { osUser = undefined }
        let dbPath; try { dbPath = store.sequelize && store.sequelize.options && store.sequelize.options.storage } catch(e: any) { /* opcional */ }
        return {
            home,
            osUser,
