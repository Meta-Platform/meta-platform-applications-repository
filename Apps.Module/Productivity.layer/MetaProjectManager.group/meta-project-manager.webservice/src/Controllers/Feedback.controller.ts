...
const FeedbackController = (params: any) => {
    const ctx = GetContext(params)
    const { store } = ctx

    const ListFeedback = async (p: any = {}) => Guard(async () => {
        await ctx.ready
    ...
    const GetFeedback = async (arg: any) => Guard(async () => {
        await ctx.ready
        return store.GetFeedback({ feedback: idOf(arg, "feedbackId") })
    })

    const CreateFeedback = async (p: any = {}) => Guard(async () => {
        await ctx.ready
