...
const ExecutionController = (params: any) => {
    ...
    const _RememberExecutable = (executableName: any, packagePath: any) => {
    ...
    // é feita PELO DAEMON.
    const RunApplication = async ({ namespaceRepo, moduleName, layerName, packageName, ext, parentGroup, executableName }: any) => {
    ...
    const StopApplication = async (executableName: any) => {
    ...
    const BuildProgressStream = async (ws: any) => {
        ...
        daemonWs.on("error",   () => { try { ws.close() } catch(e: any){} })
