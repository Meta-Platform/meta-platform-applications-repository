...
const StacksStore = ({ models, onEvent }: any) => {
    ...
    const UpdateStack = async ({ stackId, ...patch }: any) => {
    ...
    const RemoveStack = async ({ stackId }: any) => {
    ...
    const ListStackServices = async ({ stackId }: any) =>
        ListaParaJson(await models.StackService.findAll({ where: { stackId }, order: [["order", "ASC"]] }))

    const AddStackService = async ({ stackId, name, spec, dependsOn, recipeSlug, containerId, order = 0 }: any) => {
    ...
    const UpdateStackService = async ({ stackServiceId, ...patch }: any) => {
    ...
    const RemoveStackService = async ({ stackServiceId }: any) => {
    ...
    */
    const RecordComposeWritten = async ({ stackId, composeText }: any) => {
        const stack = await models.Stack.findByPk(stackId)
        if (!stack) throw CriarErro("STACK_NOT_FOUND", "Stack não encontrada.", { httpStatus: 404 })
    ...
    */
    const CheckComposeDrift = async ({ stackId, composeText }: any) => {
        const stack = await models.Stack.findByPk(stackId)
        if (!stack) throw CriarErro("STACK_NOT_FOUND", "Stack não encontrada.", { httpStatus: 404 })
