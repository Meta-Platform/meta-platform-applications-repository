...
const { NovoId, ParaJson, ListaParaJson, CriarErro } = require("./_Comum")

const RegistriesStore = ({ models, secretBox, onEvent }) => {
    ...
    const CreateRegistry = async ({ name, serverAddress, username, password, isDefault = false }) => {
    ...
    const UpdateRegistry = async ({ registryId, name, serverAddress, username, password, isDefault }) => {
    ...
    /*
        ...
        cabeçalho.
    */
    const GetAuthConfig = async ({ registryId }) => {
        const registro = await models.Registry.findByPk(registryId)
        if (!registro) throw CriarErro("REGISTRY_NOT_FOUND", "Registry não encontrado.", { httpStatus: 404 })
    ...
    */
    const FindRegistryForReference = async ({ reference }) => {
        const registros = await models.Registry.findAll()
        const texto = String(reference || "")

        const porPrefixo = registros
            .filter((r) => texto.startsWith(String(r.serverAddress).replace(/^https?:\/\//, "")))
            // O mais específico ganha: `a.com/time` antes de `a.com`.
            .sort((a, b) => String(b.serverAddress).length - String(a.serverAddress).length)[0]

        if (porPrefixo) return Publico(porPrefixo)

        const padrao = registros.find((r) => r.isDefault)
        return padrao ? Publico(padrao) : null
    }

    const RecordRegistryCheck = async ({ registryId, ok }) => {
        const registro = await models.Registry.findByPk(registryId)
    ...
    const LimparOutrosPadroes = async (registryId) => {
