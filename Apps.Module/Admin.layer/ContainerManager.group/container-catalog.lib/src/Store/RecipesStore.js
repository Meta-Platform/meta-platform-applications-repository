...
const RecipesStore = ({ models, onEvent }) => {

    const ListRecipes = async ({ category, builtin } = {}) => {
        const where = {}
    ...
    const CreateRecipe = async (receita) => {
    ...
    const UpdateRecipe = async ({ slug, ...patch }) => {
    ...
    const DuplicateRecipe = async ({ slug, newSlug, newName }) => {
    ...
    */
    ...
    const ImportRecipe = async ({ recipe, overwrite = false }) => {
    ...
    */
    const UpsertBuiltinRecipes = async ({ recipes, builtinVersion }) => {
        const resultado = { criadas: [], atualizadas: [], preservadas: [] }
