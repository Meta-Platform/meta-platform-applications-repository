...
const BuildManagedLabels = ({ origin, recipeSlug, recipeVersion, serviceId, stackName, stackService }: any) => ({
...
const ProvenanceStore = ({ models, onEvent }: any) => {
    ...
    const GetContainerProvenance = async ({ connectionId, containerId }: any) =>
        ParaJson(await models.ContainerProvenance.findOne({ where: { connectionId, containerId } }))

    const ListContainerProvenance = async ({ connectionId }: any) =>
        ListaParaJson(await models.ContainerProvenance.findAll({ where: { connectionId } }))

    const ForgetContainer = async ({ connectionId, containerId }: any) => {
    ...
    const GetImageProvenance = async ({ connectionId, imageId }: any) =>
        ParaJson(await models.ImageProvenance.findOne({ where: { connectionId, imageId } }))

    const ListImageProvenance = async ({ connectionId }: any) =>
    ...
    */
    const RecordImageUpdateCheck = async ({ connectionId, imageId, remoteDigest, updateAvailable }: any) => {
