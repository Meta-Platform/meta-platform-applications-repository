...
*/
const ProcedenciaDasLabels = (container) => {
...
const Diferencas = (specGravado, specAtual) => {
    const diferentes = []
...
const ReconcileConnection = ({ containers = [], provenance = [], specs = {} }) => {
    const porId = new Map(provenance.map((p) => [p.containerId, p]))
