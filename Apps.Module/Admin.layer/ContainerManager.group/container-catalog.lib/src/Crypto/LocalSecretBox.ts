...
/** O erro do cofre carrega um `code` estável, que quem chama testa. */
type ErroDoCofre = Error & { code: string }

const CriarErro = (code: string, message: string, causa?: any): ErroDoCofre => {
    const erro = new Error(message) as ErroDoCofre
    erro.code = code
...
/*
    ...
    já tinha selado.
*/
const GarantirChave = (keyFilePath: string): Buffer => {
...
const CreateSecretBox = ({ keyFilePath }: { keyFilePath: string }) => {
    if (!keyFilePath) {
    ...
    */
    const IsSealed = (valor: any): boolean =>
    ...
    /*
        ...
        conhecer.
    */
    const Rotate = ({ newKeyFilePath }: { newKeyFilePath?: string }) => {
