...
const ParseImageReference = require("../Helpers/ParseImageReference") as (reference: any) => {
    reference: string
...
const ImagesController = (params: any) => {
    ...
    */
    const RegistrarProcedencia = async (dados: any) => {
        try {
    ...
    const _ListImages = (connectionId: any) =>
        WithAdapter(connectionId, (adaptador: any) => adaptador.ListAllImages())

    const _InspectImage = ({ connectionId, imageIdOrName }: any) =>
        WithAdapter(connectionId, (adaptador: any) => adaptador.InspectImage(imageIdOrName))
    ...
    */
    const _GetImageHistory = ({ connectionId, imageIdOrName }: any) =>
        WithAdapter(connectionId, (adaptador: any) => adaptador.GetImageHistory(imageIdOrName))

    const _SearchImages = ({ connectionId, term, limit }: any) =>
        WithAdapter(connectionId, (adaptador: any) => adaptador.SearchImages({ term, limit }))

    /* ---------------------------------------------------------- remoção */

    const _RemoveImage = async ({ connectionId, imageIdOrName, force = false }: any) => {
    ...
    const _PruneImages = async ({ connectionId, dangling = true }: any) => {
        const resultado = await WithAdapter(connectionId, (adaptador: any) =>
    ...
    */
    const _PullImage = async ({ connectionId, reference, platform, registryId }: any) => {
        const auth = await ResolverCredencial({ registryId, reference })

        const inspecao = await WithAdapter(connectionId, (adaptador: any) =>
    ...
    const GravarProcedenciaDePull = async ({ connectionId, reference, inspecao }: any) => {
    ...
    */
    const _TransferStream = async (ws: any, { connectionId }: any) => {
        let cancelado = false
        let ocupado = false
        ...
        const Fechar = () => { try { ws.close() } catch(erro: any) { /* já fechado */ } }

        ws.on("close", () => { cancelado = true })
        ws.on("error", () => { cancelado = true })

        const AoProgredir = (evento: any) => {
            if (cancelado) return
            Enviar({ type: "progress", ...evento })
        }

        const Baixar = async ({ reference, platform, registryId }: any) => {
            const auth = await ResolverCredencial({ registryId, reference })
        ...
        const Enviar_ = async ({ reference, tag, registryId }: any) => {
            const auth = await ResolverCredencial({ registryId, reference })
    ...
    const _TagImage = ({ connectionId, imageIdOrName, repo, tag }: any) =>
        WithAdapter(connectionId, (adaptador: any) => adaptador.TagImage({ imageIdOrName, repo, tag }))
    ...
    */
    const _PushImage = async ({ connectionId, reference, tag, registryId }: any) => {
        const auth = await ResolverCredencial({ registryId, reference })
        const resultado = await WithAdapter(connectionId, (adaptador: any) =>
    ...
    */
    const _ExportImage = async ({ connectionId, imageIdOrName }: any) => {
        const inspecao = await WithAdapter(connectionId, (a: any) => a.InspectImage(imageIdOrName))
        EnsureFits({ sizeBytes: inspecao.Size, what: `A imagem ${imageIdOrName}` })

        return await WithAdapter(connectionId, (a: any) => a.ExportImage(imageIdOrName))
    }

    const _LoadImage = async ({ connectionId, contentBase64 }: any) => {
        ...
        const resultado = await WithAdapter(connectionId, (a: any) => a.LoadImage({ contentBase64 }))

        for (const nome of resultado.loaded || []) {
            try {
                const inspecao = await WithAdapter(connectionId, (a: any) => a.InspectImage(nome))
    ...
    */
    const _CheckImageUpdate = async ({ connectionId, reference, registryId }: any) => {
        const auth = await ResolverCredencial({ registryId, reference })
        const resultado = await WithAdapter(connectionId, (a: any) => a.CheckImageUpdate({ reference, auth }))
    ...
    */
    const _CheckAllImageUpdates = async ({ connectionId, references }: any) => {
        const alvos = references && references.length > 0
            ? references
            : (await WithAdapter(connectionId, (a: any) => a.ListAllImages()))
                .flatMap((imagem: any) => imagem.RepoTags || [])
                .filter((tag: any) => tag && tag !== "<none>:<none>")
    ...
    const _GetImageProvenance = async ({ connectionId, imageId }: any) => {
        const store = await contexto.GetStoreOrNull()
        if (!store) return null
        return await store.GetImageProvenance({ connectionId, imageId })
    }

    const _ListImageProvenance = async (connectionId: any) => {
        const store = await contexto.GetStoreOrNull()
        if (!store) return []
    ...
    /*
        ...
        resposta possível.
    */
    const _BuildImage = ({ connectionId, imageTagName, dockerfileContent, buildargs }: any) =>
        WithAdapter(connectionId, async (adaptador: any) => {
            const log: string[] = []

            const Coletar = (pedaco: any) => {
