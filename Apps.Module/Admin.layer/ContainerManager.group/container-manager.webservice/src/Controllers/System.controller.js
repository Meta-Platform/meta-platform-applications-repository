...
const SystemController = ({
    ...
    const _EventsStream = async (ws, { connectionId }) => {
        let assinatura = null
        ...
        const Interessa = (evento) => {
            ...
            const tipos = filtros.type ? [].concat(filtros.type) : null
            const acoes = filtros.action ? [].concat(filtros.action) : null
        ...
        const Encerrar = () => {
            ...
            try { assinatura && assinatura.Close() } catch (error) { /* já fechado */ }
            assinatura = null
        }
        ...
        try {
            assinatura = await WithAdapter(connectionId, (adaptador) =>
                adaptador.StreamRuntimeEvents({
                    onData: Enfileirar,
                    onError: (error) => _EnviarNoSocket(ws, { type: "error", message: error.message })
                }))
    ...
    const GetRuntimeInfo = async ({ connectionId }) =>
        await WithAdapter(connectionId, (a) => a.GetRuntimeInfo())

    const GetRuntimeVersion = async ({ connectionId }) =>
        await WithAdapter(connectionId, (a) => a.GetRuntimeVersion())

    const PingRuntime = async ({ connectionId }) =>
        await WithAdapter(connectionId, (a) => a.PingRuntime())

    const GetDiskUsage = async ({ connectionId }) =>
        await WithAdapter(connectionId, (a) => a.GetDiskUsage())
    ...
    */
    const PrunePreview = async ({ connectionId, types, danglingOnly = true }) => {
        const escolhidos = types && types.length > 0
            ? types
            : ["containers", "images", "networks", "volumes"]

        return await WithAdapter(connectionId, async (adaptador) => {
            const perType = {}
            ...
            if (escolhidos.includes("containers")) {
                ...
                perType.containers = {
                    ...
                    totalBytes: parados.reduce((t, c) => t + (c.SizeRw || 0), 0)
                }
            }
            ...
            if (escolhidos.includes("images")) {
                ...
                perType.images = {
                    ...
                    totalBytes: imagens.reduce((t, i) => t + (i.Size || 0), 0)
                }
            }
            ...
            if (escolhidos.includes("networks")) {
                ...
                const podaveis = redes.filter((r) => !["bridge", "host", "none"].includes(r.Name))
                perType.networks = {
                    items: podaveis.map((r) => ({ id: r.Id, name: r.Name, size: 0, reason: "sem container conectado" })),
                    totalBytes: 0
                }
            }
            ...
            if (escolhidos.includes("volumes")) {
                ...
                perType.volumes = {
                    ...
                    totalBytes: Volumes.reduce((t, v) => t + (v.UsageData?.Size ?? 0), 0)
                }
            }
    ...
    const PruneSystem = async ({ connectionId, types, danglingOnly = true, filters }) => {
            ...
            await store.RecordActivity({
                ...
                details: {
                    ...
                    perType: Object.fromEntries(
                        Object.entries(resultado.perType).map(([t, r]) => [t, r.deleted.length])
                    ),
    ...
    */
    const ListOrphans = async ({ connectionId, stoppedForDays = 7 }) =>
        await WithAdapter(connectionId, async (adaptador) => {
            ...
            const imagensEmUso = new Set(containers.map((c) => c.ImageID))
            ...
            return {
                volumes: volumes
                    .filter((v) => !emUso.has(v.Name))
                    .map((v) => ({ name: v.Name, size: v.UsageData?.Size ?? null, reason: "nenhum container monta" })),

                networks: redes
                    .filter((r) => !["bridge", "host", "none"].includes(r.Name))
                    .filter((r) => Object.keys(r.Containers || {}).length === 0)
                    .map((r) => ({ id: r.Id, name: r.Name, reason: "nenhum container conectado" })),

                images: imagens
                    .filter((i) => !imagensEmUso.has(i.Id))
                    .filter((i) => !i.RepoTags || i.RepoTags.length === 0 || i.RepoTags[0] === "<none>:<none>")
                    .map((i) => ({ id: i.Id, size: i.Size, reason: "sem tag e sem container" })),

                containers: containers
                    .filter((c) => c.State !== "running" && (c.Created * 1000) < limite)
                    .map((c) => ({
                        id: c.Id,
                        name: (c.Names?.[0] || "").replace(/^\//, ""),
                ...
                services: servicos
                    .filter((s) => s.containerId && !idsNoRuntime.has(s.containerId))
                    .map((s) => ({ serviceId: s.id, name: s.name, reason: "o container não existe mais" }))
            }
        })

    /* ------------------------------------------------------------- trilha */

    const ListActivity = async ({ connectionId, action, targetId, limit, offset }) => {
