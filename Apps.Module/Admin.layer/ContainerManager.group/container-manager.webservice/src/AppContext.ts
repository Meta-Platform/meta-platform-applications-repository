...
*/
const DerivarCaminhos = ({ appDataDir, dbFilePath, secretKeyPath, stacksDir }: any) => {
...
const GetContext = (parametros: any = {}) => {
    ...
    let store: any = null
    let ready: Promise<any> = Promise.resolve(null)
    ...
    if (containerCatalogLib && dbFilePath) {
        try {
            ...
            ready = store.ConnectAndSync().catch((erro: any) => {
                /*
