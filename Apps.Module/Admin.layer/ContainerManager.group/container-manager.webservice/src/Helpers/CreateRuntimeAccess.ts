...
const DescreveFalhaDeConexao = (error: any) => {
...
class RuntimeUnavailableError extends Error {
    ...
    // Declarados porque são atribuídos aqui e LIDOS lá fora — pela borda HTTP,
    // que tira daqui o status da resposta.
    code: string
    httpStatus: number
    statusCode: number
    constructor(conexao: any, causa: any) {
        const onde = conexao
...
const CreateRuntimeAccess = ({ containerRuntimeConnectionService }: any) => {
    ...
    */
    const WithAdapter = async (connectionId: any, Operation: any) => {
