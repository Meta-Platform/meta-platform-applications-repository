...
const InterpretarTamanho = (valor: any) => {
    ...
    const multiplicador = ({ "": 1, k: 1024, m: 1024 ** 2, g: 1024 ** 3, t: 1024 ** 4 } as Record<string, number>)[casou[2]]
...
*/
const EnsureFits = ({ sizeBytes, what = "o conteúdo", limitBytes }: any) => {
    const limite = limitBytes || MaxInlineBytes()
    if (!sizeBytes || sizeBytes <= limite) return { ok: true, limitBytes: limite }

    // Os campos abaixo são lidos pela borda HTTP (status) e pela tela (tamanhos).
    ...
    ) as Error & { code: string, httpStatus: number, statusCode: number, sizeBytes: number, limitBytes: number }
