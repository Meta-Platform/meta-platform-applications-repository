...
const RelacionalDatabaseHandlerController = (params: any) => {

    const { dataSourceLocalService } = params

    const _GetSource = (keystone: any) => {
    ...
    const _Quote = (connection: any, id: any) => {
    ...
    const _MapType = (connection: any, typeStr: any) => {
    ...
    const _BuildAttribute = (connection: any, column: any) => {
    ...
    const _ShowAllTableName = ({ keystone }: any) =>
        _Ctx(keystone).then(async ({ qi }) => {
            const tables = await qi.showAllTables()
            return tables.map((t: any) => (typeof t === "string" ? t : (t.tableName || Object.values(t)[0])))
        })

    const _DescribeTable = ({ keystone, tableName }: any) =>
        _Ctx(keystone).then(async ({ qi }) => {
            const desc = await qi.describeTable(tableName)
            return Object.keys(desc).map((name) => ({ name, ...desc[name] }))
        })

    const _ShowTableIndexes = ({ keystone, tableName }: any) =>
        _Ctx(keystone).then(({ qi }) => qi.showIndex(tableName))

    // --- Consulta ------------------------------------------------------------

    const _CountRows = ({ keystone, tableName }: any) =>
    ...
    const _SelectRows = ({ keystone, tableName, limit, offset, orderBy, orderDir }: any) =>
    ...
    const _RunSQL = ({ keystone, sql }: any) =>
        _Ctx(keystone).then(async ({ connection }) => {
    ...
    const _InsertRow = ({ keystone, tableName, values }: any) =>
        _Ctx(keystone).then(async ({ qi }) => {
            await qi.bulkInsert(tableName, [values])
            return { inserted: 1 }
        })

    const _UpdateRow = ({ keystone, tableName, values, where }: any) =>
        _Ctx(keystone).then(async ({ qi }) => {
            await qi.bulkUpdate(tableName, values, where)
            return { updated: true }
        })

    const _DeleteRow = ({ keystone, tableName, where }: any) =>
        _Ctx(keystone).then(async ({ qi }) => {
    ...
    const _CreateTable = ({ keystone, tableName, columns }: any) =>
        _Ctx(keystone).then(async ({ connection, qi }) => {
            const attributes: Record<string, any> = {}
            ;(columns || []).forEach((c: any) => { attributes[c.name] = _BuildAttribute(connection, c) })
            await qi.createTable(tableName, attributes)
            return { created: tableName }
        })

    const _DropTable = ({ keystone, tableName }: any) =>
        _Ctx(keystone).then(async ({ qi }) => {
            await qi.dropTable(tableName)
            return { dropped: tableName }
        })

    const _AddColumn = ({ keystone, tableName, column }: any) =>
        _Ctx(keystone).then(async ({ connection, qi }) => {
            await qi.addColumn(tableName, column.name, _BuildAttribute(connection, column))
            return { added: column.name }
        })

    const _RemoveColumn = ({ keystone, tableName, columnName }: any) =>
        _Ctx(keystone).then(async ({ qi }) => {
