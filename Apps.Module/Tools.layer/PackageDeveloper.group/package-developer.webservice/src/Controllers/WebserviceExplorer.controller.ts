...
const WebserviceExplorerController = (params: any) => {
    ...
    const _GetDetails = ({packageName, workspace, ext}: any) => {
    ...
    const _GetBoot = ({packageName, workspace, ext}: any) => {
        const packageDevelopmentService = GetPackage(packageHandlerManagerService, {packageName, workspace, ext})
        if(!packageDevelopmentService) return undefined
        const webService = GetWebserviceService(packageDevelopmentService)
    ...
    const _GetAPIs = ({packageName, workspace, ext}: any) => {
        const packageDevelopmentService = GetPackage(packageHandlerManagerService, {packageName, workspace, ext})
        if(!packageDevelopmentService) return undefined
        const webService = GetWebserviceService(packageDevelopmentService)
        return webService.GetAPIs()
    }
    const _GetControllers = ({packageName, workspace, ext}: any) => {
        const packageDevelopmentService = GetPackage(packageHandlerManagerService, {packageName, workspace, ext})
        if(!packageDevelopmentService) return undefined
        const webService = GetWebserviceService(packageDevelopmentService)
