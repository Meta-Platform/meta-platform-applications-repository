...
const PackageTasksController = (params: any) => {
    ...
    const _resolvePackage = ({packageName, type, workspace}: any) => {
    ...
    const _id = ({workspace, packageName, type}: any) => `${workspace}/${packageName}.${type}`

    const _InstallDependencies = async ({packageName, type, workspace}: any) => {
    ...
    const _Debug = ({packageName, type, workspace}: any) => {
        const pkg = _resolvePackage({packageName, type, workspace})
        return processManagerService.StartPackage({
            id: _id({workspace, packageName, type}),
            packagePath: pkg.path,
    ...
    const _Console = (ws: any, {workspace, packageName, type}: any) => {
        const id = _id({workspace, packageName, type})

        const safeSend = (entry: any) => {
            try { ws.send(JSON.stringify(entry)) } catch(e: any) { /* socket fechado */ }
        }
