...
const FileSystemNavigatorController = (params: any) => {
    ...
    const _AssertInsidePackage = (storePath: any, itemPath: any) => {
    ...
    const _ListItem = ({packageName, workspace, ext, path}: any) =>  {
    ...
    const _GetContentItem = ({packageName, workspace, ext, path}: any) => {
    ...
    const _SaveContentItem = ({packageName, workspace, ext, path, content}: any) => {
        const packageDevelopmentService = GetPackage(packageHandlerManagerService, {packageName, workspace, ext})
    ...
    const _GetPackageMetadata = ({packageName, workspace, ext}: any) => {
        const packageDevelopmentService = GetPackage(packageHandlerManagerService, {packageName, workspace, ext})
        if(!packageDevelopmentService) return undefined
        return GetPackageMetadata(packageDevelopmentService)
    }

    const _CreateContentItem = ({packageName, workspace, ext, path: itemPath, content}: any) => {
        const packageDevelopmentService = GetPackage(packageHandlerManagerService, {packageName, workspace, ext})
        if(!packageDevelopmentService) throw `Pacote "${packageName}" (${ext}) não encontrado na workspace "${workspace}"`
    ...
    const _RenameContentItem = ({packageName, workspace, ext, path: fromPath, newPath: toPath}: any) => {
        const packageDevelopmentService = GetPackage(packageHandlerManagerService, {packageName, workspace, ext})
        if(!packageDevelopmentService) throw `Pacote "${packageName}" (${ext}) não encontrado na workspace "${workspace}"`
    ...
    const _DeleteContentItem = ({packageName, workspace, ext, path: itemPath}: any) => {
        const packageDevelopmentService = GetPackage(packageHandlerManagerService, {packageName, workspace, ext})
        if(!packageDevelopmentService) throw `Pacote "${packageName}" (${ext}) não encontrado na workspace "${workspace}"`
        _AssertInsidePackage(packageDevelopmentService.path, itemPath)
    ...
    const _SearchFiles = async ({packageName, workspace, ext, query, path: rootPath}: any) => {
        ...
        const walk = async (dir: any) => {
            ...
            let items
            try { const r = await ListItem(service, dir || "/"); items = (r && r.listItem) || [] } catch(e: any) { return }
            for(const it of items){
                ...
                if(TEXT_EXT.test(it.filename)){
                    let content
                    try { content = await GetContentItem(service, p) } catch(e: any) { content = undefined }
