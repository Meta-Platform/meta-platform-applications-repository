...
const PACKAGE_CREATORS: Record<string, string> = {
    lib     : "CreateLibPackage",
    cli     : "CreateCliPackage",
    service : "CreateServicesPackage"
}

const ModuleDeveloperController = (params: any) => {
    const {
    ...
    const _ListRecentRepositories = (limit: any) =>
    ...
    const _CreateWorkspace = async ({ name, path }: any) => {
    ...
    const _CreateRepository = async ({ name, path: parentPath }: any) => {
    ...
    const _GetRepositoryIndex = async (name: any) => {
        const repo = await packageHandlerManagerService.GetWorkspace({ name })
        if(!repo) throw `Repository "${name}" não encontrado`
    ...
    const _GetRepositoryMetadata = async (name: any) => {
        const repo = await packageHandlerManagerService.GetWorkspace({ name })
        if(!repo) throw `Repository "${name}" não encontrado`
    ...
    const _BrowseDir = async (arg: any) => {
        ...
        const entries = (await readdir(target, { withFileTypes: true })).filter((e: any) => e.isDirectory())
        const directories = await Promise.all(entries.map(async (entry: any) => {
    ...
    const _GetAppState = (key: any) => packageHandlerManagerService.GetState(key)
    const _SetAppState = ({ key, value }: any) => packageHandlerManagerService.SetState(key, value)

    // Garante que um caminho-alvo está dentro do repositório (segurança).
    const _AssertInsideRepo = (repoPath: any, targetPath: any) => {
    ...
    const _CreateContainer = async ({ workspace, parentPath, name, kind }: any) => {
    ...
    const _CreatePackage = async ({ workspace, targetPath, packageName, ext }: any) => {
        const targetWorkspace = await packageHandlerManagerService.GetWorkspace({ name: workspace })
        if(!targetWorkspace) throw `Workspace "${workspace}" não encontrada`
    ...
    const _RenameNode = async ({ workspace, path: nodePath, newName }: any) => {
        const targetWorkspace = await packageHandlerManagerService.GetWorkspace({ name: workspace })
        if(!targetWorkspace) throw `Workspace "${workspace}" não encontrada`
    ...
    const _DeleteNode = async ({ workspace, path: nodePath }: any) => {
        const targetWorkspace = await packageHandlerManagerService.GetWorkspace({ name: workspace })
        if(!targetWorkspace) throw `Workspace "${workspace}" não encontrada`
        const safe = _AssertInsideRepo(targetWorkspace.path, nodePath)
    ...
    const _ListPackagesByWorkspace = (workspaceName: any) =>
    ...
    const _GitStatusStream = async (ws: any, arg: any) => {
        const raw = (arg && (arg.repositories !== undefined ? arg.repositories : arg)) || ""
        const names = String(raw).split(",").map((s) => s.trim()).filter(Boolean)

        if(!gitStatusManagerService){
            try { ws.send(JSON.stringify({ statusByPath: {}, repositories: {} })) } catch(e: any) {}
            return
        }
        ...
        for(const name of names){
            ...
            } catch(e: any) { /* workspace inexistente */ }
        }

        const safeSend = async () => {
            try { ws.send(JSON.stringify(await subscription.GetStatus())) } catch(e: any) { /* socket fechado */ }
        }
        const subscription = gitStatusManagerService.Subscribe(repoList, () => { safeSend() })
        if(ws.on) ws.on("close", () => { try { subscription.dispose() } catch(e: any) {} })
        safeSend()   // snapshot inicial
    }

    const _GetIcon = ({packageName, workspace, ext}: any) => {
        try{
