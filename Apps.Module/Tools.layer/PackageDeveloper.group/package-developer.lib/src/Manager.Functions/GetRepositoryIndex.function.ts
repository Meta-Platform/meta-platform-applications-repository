...
const FileExists = async (filePath: any) => {
    try { await fs.promises.access(filePath); return true } catch(e: any) { return false }
}
...
const ReadPackageJson = async (packagePath: any) => {
...
const ReadPackageMetadata = async (packagePath: any) => {
    ...
    for (const filename of fileNames.filter((f: any) => CAPABILITY_FILES.indexOf(f) > -1))
        metadata[`metadata/${filename}`] = await ReadJson(path.resolve(metadataDir, filename))

    return { metadataFiles: fileNames.map((f: any) => `metadata/${f}`), metadata }
}

const HasDirectory = async (packagePath: any, dirName: any) => {
    try { return (await fs.promises.stat(path.resolve(packagePath, dirName))).isDirectory() } catch(e: any) { return false }
}
...
const CollectPackages = (hierarchy: any) => {
    const out = []
...
const GetRepositoryIndexFunction = async (repositoryPath: any) => {
    ...
    const layerCount  = (hierarchy.modules || []).reduce((n: any, m: any) => n + (m.layers || []).length, 0)
    const groupCount  = (hierarchy.modules || []).reduce((n: any, m: any) =>
        n + (m.layers || []).reduce((k: any, l: any) => k + (l.groups || []).length, 0), 0)
