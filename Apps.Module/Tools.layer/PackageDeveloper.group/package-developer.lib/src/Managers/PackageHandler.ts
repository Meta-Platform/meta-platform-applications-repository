...
const isContainer = (name: any) => CONTAINER_SUFFIXES.some((suffix) => name.endsWith(suffix))
...
const GetAllServiceParamsByPath = async (rootPath: any, depth = 0): Promise<any[]> => {
    let entries
    try{
...
const PackageHandlerManager = (params: any) => {
    ...
    const _LoadService = (serviceParams: any) =>  {
    ...
    const _LoadWorkspace = async ({ name, path }: any) => {
        try{
            const listServiceParams = await GetAllServiceParamsByPath(path)
            listServiceParams
             .forEach((serviceParams: any) => _LoadService({ ...serviceParams, workspaceName: name }))
        }catch(e: any){
            if(e.code === "ENOENT"){
    ...
    const _ListRecentWorkspaces = (limit: any) => store.ListRecent(limit)

    const _TouchWorkspace = ({ name }: any) => store.Touch({ name })

    const _GetState = (key: any) => store.GetState(key)
    const _SetState = (key: any, value: any) => store.SetState(key, value)

    const _GetWorkspace = ({ name }: any) => store.Get({ name })

    // Re-varre os pacotes de uma workspace existente (após criar/remover pacote).
    const _ReloadWorkspace = async ({ name }: any) => {
    ...
    const _RemoveWorkspace = async ({ name }: any) => {
